...
struct airoha_nfc {
	...
	bool data_inv;
	/* A trial read in another format: failing to decode is the answer */
	bool quiet;
	/*
	 * Failing pages, reported a line a second at most (see
	 * airoha_nfc_bad_page()): whether one has been, when the last line
	 * went out, and the failing page reads held back since, over pages
	 * bad_lo-bad_hi.
	 */
	bool bad_shown;
	ulong bad_time;
