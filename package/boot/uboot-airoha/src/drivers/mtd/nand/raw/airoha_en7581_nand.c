...
/*
 * Failing pages are reported a line a second at most: one by itself, and
 * those after it within the second counted, and summed up by the next
 * read once the second is over.  Flash in another ECC layout fails every
 * sector of every page, and whoever reads it -- a UBI attach reads two
 * pages of each of 2048 eraseblocks, UBI reads again UBI_IO_RETRIES times
 * -- would otherwise fill the console for minutes at 115200 baud, a line
 * saying what the count says.
 */
static void airoha_nfc_bad_sum(struct airoha_nfc *nfc)
{
	if (!nfc->bad_held || get_timer(nfc->bad_time) < 1000)
		return;
	pr_warn("Uncorrectable ECC error on %d more page reads, pages %d-%d\n",
		nfc->bad_held, nfc->bad_lo, nfc->bad_hi);
	nfc->bad_held = 0;
	nfc->bad_time = get_timer(0);
}

static void airoha_nfc_bad_page(struct airoha_nfc *nfc, int page, int failed,
				int steps)
{
	if (nfc->bad_shown && get_timer(nfc->bad_time) < 1000) {
		if (!nfc->bad_held++)
			nfc->bad_lo = nfc->bad_hi = page;
		nfc->bad_lo = min(nfc->bad_lo, page);
		nfc->bad_hi = max(nfc->bad_hi, page);
		return;
	}
	airoha_nfc_bad_sum(nfc);
	pr_warn("Uncorrectable ECC error at page %d (%d of %d sectors)\n",
		page, failed, steps);
	nfc->bad_shown = true;
	nfc->bad_time = get_timer(0);
}
				      ...
				      int oob_required, int page)
{
	...
	if (!nfc->quiet) {
		if (failed)
			airoha_nfc_bad_page(nfc, page, failed, nand->ecc.steps);
		else
			airoha_nfc_bad_sum(nfc);
	}
out:
