...
static int refresh_boot_menu(void)
{
	...
	/*
	 * Loaded over the serial port, U-Boot writes nothing by itself: the
	 * refresh holds for this boot, and the U-Boot on flash does its own.
	 */
	if (airoha_ramboot()) {
		printf("Environment not saved: this U-Boot was loaded over the serial port\n");
		return 0;
	}
