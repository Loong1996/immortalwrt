...
/* @@PAGE_BEGIN@@ */
	...
	"* 来、失败时设备自己会回到恢复页。再盖一个「设备正在启动系统」只是挡住它。\n"
	"* 所以这段时间一个框都不弹；设备真回来了就整页刷新 —— 那正是 p7 上承诺的\n"
	"* 那一行，而 bootm 失败是原地返回、uptime 不会倒退，靠不了那条判据。\n"
	"*/\n"
	"var TRYB=0;\n"
	...
	"var NETINIT=0;\n"
	"var NET=null,NETT=null;\n"
	"/*\n"
	"* 擦除加写入，SPI-NAND 上大致这个量级，只用于估算等待时间。设备写完会报\n"
	"* 一次实测速度，存下来给下次用 —— 同一块板子第二次起就不是拍脑袋了。\n"
	"*/\n"
	"var WRSPD=1400000;\n"
	"try{var wr0=+localStorage.getItem('xgwrspd');if(wr0>0)WRSPD=wr0}catch(e){}\n"
	"var WR=null,WRT=null,WRP=null;\n"
	...
	"function sz(n){return n>=1048576?(n/1048576).toFixed(1)+' MiB':n>=1024?(n/1024).toFixed(0)+'"
		" KiB':n+' B'}\n"
	"function spd(b){return b>=1048576?(b/1048576).toFixed(1)+' MiB/s':b>=1024?(b/1024).toFixed(0)+'"
		" KiB/s':Math.round(b)+' B/s'}\n"
	"/*\n"
	"* 秒数说成人话；超过一小时就不必精确到秒了。这一处不走字典 —— 「 分」「 秒」\n"
	"* 当片段太短，会咬进「擦除 UBI 分区」这种词里，所以在这里直接分语言。\n"
	"*/\n"
	"function dur(s){s=Math.round(s);var e=LANG=='en';\n"
	"if(s<60)return s+(e?' s':' 秒');\n"
	"var m=Math.floor(s/60);if(m<60)return m+(e?' min ':' 分 ')+(s%60)+(e?' s':' 秒');\n"
	"return Math.floor(m/60)+(e?' h ':' 小时 ')+(m%60)+(e?' min':' 分')}\n"
	...
	"/* 同一屏里并列的几组：一次只显示一组，其余留在 DOM 里不动 */\n"
	"function seg(b){var w=b.parentNode,g=w.getAttribute('data-seg'),\n"
	"id=b.getAttribute('data-s');\n"
	"$$('button',w).forEach(function(x){\n"
	"x.setAttribute('aria-selected',x==b?'true':'false')});\n"
	"$$('.sp[data-g='+g+']').forEach(function(p){\n"
	"if(p.id==id)p.setAttribute('data-on','');else p.removeAttribute('data-on')});\n"
	"/* 跟随开着的时候整篇重读会和增量追加叠出重复，而且会把按钮解禁 */\n"
	"if(g=='g8'){if(id=='s83'){if(!$('#logf').checked)getlog()}else logstop()}\n"
	"if(g=='g5')netpoll(id=='s52')}\n"
	...
	"* 现在它一边写一边服务，侧栏里的改地址、恢复默认环境变量、重启这几项点下去\n"
	"* 是在写到一半的闪存上动手。设备那边各自回 503 挡了一道，这里是别让人碰到。\n"
	"* 光有 pointer-events 不够：键盘 Tab 过去按回车照样走得通。\n"
	"*/\n"
	"function busy(on){var a=$('#app');\n"
	"if(on)a.setAttribute('data-busy','');else a.removeAttribute('data-busy');\n"
	"$$('.nav').forEach(function(b){b.disabled=!!on})}\n"
	...
	"function refresh(){var p=pane(),st=$('.st',p);if(!st)return;var"
		...
		" '+fs.length+' 个文件 · '+sz(n):'未选择文件'}\n"
	"/* 每个文件框都能拖：大框是 .drop，表格里的是它所在的 .fr */\n"
	"function drag(d,i){if(!d||d.__drag)return;d.__drag=1;\n"
	...
	"if(e.dataTransfer&&e.dataTransfer.files.length){try{i.files=e.dataTransfer.files;onfile(i)}catch"
		"(x){}}}}\n"
	"function bind(){$$('input[type=file]').forEach(function(i){i.onchange=function(){onfile(i)};\n"
	"drag(i.closest?(i.closest('.drop')||i.closest('.fr')):null,i)})}\n"
	"function hex(v){v=(v||'').trim();if(!/^(0x)?[0-9a-fA-F]{1,8}$/.test(v))return-1;return"
		" parseInt(v,16)}\n"
	"/*\n"
	"* 擦净尾部擦到的是片尾，不是分区尾 —— 设备那边 st_wipe_end 就是 flash 容量。\n"
	"* 偏移 0 是「整片写回」，把镜像之后的残留擦掉是对的；偏移非 0 是往单个分区里\n"
	"* 写，再擦就把它后面的东西一起带走了：按页面自己教的「备份下载→原始区段」\n"
	"* 导出一段再写回来，默认勾着就会连 UBI 和 ri/bosa 一起擦空。所以偏移一改就\n"
	"* 松勾。用户亲手动过这个开关之后不再代他决定 —— dataset.t 记的就是这个。\n"
	"*/\n"
	"function stwipe(){var w=$('[name=wipe]');if(w&&!w.dataset.t)w.checked=hex($('[name=stockoff]').v"
		"alue)===0}\n"
	...
	"* 口亮着不值当。所以只报地址与端口的那一半单独开在 /net，网络这一页在前台\n"
	"* 时每 3 秒问一次，切走就停。\n"
	"*/\n"
	"function netget(){if(GONE)return;get('/net',function(st,t){var r=null;\n"
	"try{r=JSON.parse(t)}catch(e){}\n"
	"if(!r||!r.net)return;NET=r;\n"
	"if(INFO){INFO.net=r.net;INFO.ports=r.ports}\n"
	"netfill()})}\n"
	...
	"function hbstart(){HB=1;ping()}\n"
	"/* 代数：停一次就换一代，飞行中的那次 ping 回来时对不上，直接丢掉 */\n"
	"function hbstop(){HB=0;HBGEN++;clearTimeout(HBT);HBT=null}\n"
	"function hbnext(){if(!HB)return;clearTimeout(HBT);HBT=setTimeout(ping,LIVE?3000:2000)}\n"
	"function ping(){if(!HB)return;if(HBOFF){hbnext();return}\n"
	"var g=HBGEN,x=new XMLHttpRequest(),fired=0,fin=function(o,u){if(fired)return;fired=1;if(g!=HBGEN"
		")return;seen(o,u);hbnext()};\n"
	"x.timeout=2500;\n"
	"x.onload=function(){var u=-1,r=null;try{r=JSON.parse(x.responseText)}catch(e){}\n"
	"if(r){if(r.up!==undefined)u=r.up;$('#logovf').hidden=!r.ovf}\n"
	"/* 200 不算凭据：那个地址上随便什么东西都能回 200 —— 电脑另一条网卡后面的\n"
	"路由器、浏览器缓存、中间的代理。认这块板子的是 up 这个字段 */\n"
	"fin(x.status==200&&u>=0,u)};\n"
	"x.onerror=function(){fin(0,-1)};x.ontimeout=function(){fin(0,-1)};\n"
	"x.open('GET','/ping');x.send()}\n"
	"function seen(o,u){\n"
	"if(o){SILENT=0;if(!LIVE){if(TRYB||OFFWHY=='reboot'||(UP>=0&&u>=0&&u<UP)){location.reload();retur"
		"n}LIVE=1;online()}\n"
	"MISS=0;if(u>=0)UP=u;live(0);return}\n"
	"MISS++;\n"
	"/*\n"
	"* 自己发起的读取（体检、备份）会让设备静默几十秒。那一页上已经写着在读什么，\n"
	"* 再盖一个框纯是噪音 —— 只把左下角的点变掉。真的很久不回来才退回通用提示。\n"
	"*/\n"
	...
	"$('#offb').innerHTML='<div class=w style=\"color:inherit\">'+b+'</div>'+\n"
	...
	"function reconnect(){var b=$('#offr');b.disabled=true;b.textContent='正在重试…';clearTimeout(HBT);if"
		"(!HB)HB=1;ping()}\n"
	"/*\n"
	"* 「闪存内容不受影响」单说这一句会误导：闪存没变正是问题所在 —— 当前这个\n"
	"* U-Boot 只在内存里（首次迁移经串口传进来，或 fip 卷已丢），reset 之后就没了。\n"
	...
	"var y=$('#yes');y.hidden=false;y.textContent='立即重启';YES=doreboot;$('#mask').setAttribute('data-o"
		"n','')}\n"
	"function doreboot(){get('/reboot',function(){});expect('reboot')}\n"
	"/*\n"
	"* 一个下拉框，三种模式，互斥：本机发地址 / 本机用固定地址 / 向上级路由要\n"
	"* 地址。原来是「静态 or 自动获取」再叠一个独立的 DHCP 服务开关 —— 两个维度\n"
	"* 四种组合，其中「自动获取 + 服务开着」是纯粹的错，只能靠代码在背后偷偷关\n"
	"* 掉，而「关掉」这件事又存不进闪存。合成一维之后，那种组合根本表达不出来。\n"
	"*\n"
	"* DHCP 服务器那一档掩码固定 /24：设备发出去的地址一直是按 /24 算的\n"
	"* （net_ip & 0xffffff00 | 100），能改掩码只会让它和实际发出去的地址对不上。\n"
	"*/\n"
	...
	"ippv();bootline()}\n"
	"/* 末位固定 .1 是这一档的规矩，那就当场把结果摆出来，别等按了「应用」才说 */\n"
	"function ippv(){var m=$('#amode').value,ip=$('#nip').value.trim(),f=$('#ipfn'),b;\n"
	"if(m=='client'){f.textContent='';return}\n"
	"if(m=='static'){f.textContent='主机需在同一网段';return}\n"
	"if(!IP4.test(ip)){f.textContent='填本网段内任一地址即可，如 192.168.1.1';return}\n"
	"b=ip.replace(/\\.\\d+$/,'');\n"
	"f.innerHTML='设备 <b>'+esc(b)+'.1</b> · 电脑 <b>'+esc(b)+'.100</b>'}\n"
	"/* 闪存里存着的那一套；从没存过就是出厂默认 */\n"
	"function bootdesc(){var v=INFO&&INFO.net&&INFO.net.saved;\n"
	"if(!v)return 'DHCP 服务器 192.168.1.1 / 255.255.255.0（出厂默认）';\n"
	"return MODES[v.mode]+(v.mode=='client'?'':' '+v.ip\n"
	"+(v.mode=='static'?' / '+v.mask:''))}\n"
	"function bootline(){$('#nboot').innerHTML='<b>下次开机：'+esc(bootdesc())\n"
	"+'</b>。勾上「保存到闪存」才会把当前这一套写进去替换它；不勾就只管本次开机。'}\n"
	"var IP4=/^(\\d{1,3}\\.){3}\\d{1,3}$/;\n"
	"function applyaddr(){var m=$('#amode').value,ip=$('#nip').value.trim(),\n"
	"mk=m=='server'?'255.255.255.0':($('#nmask').value.trim()||'255.255.255.0'),\n"
	"sv=$('#nsave').checked,h;\n"
	"if(m!='client'){\n"
	"if(!IP4.test(ip)){$('#nh').textContent='IP 不是一个合法的地址';return}\n"
	"if(m=='server')ip=ip.replace(/\\.\\d+$/,'.1');\n"
	"else if(!IP4.test(mk)){$('#nh').textContent='子网掩码不是一个合法的地址';return}}\n"
	"$('#atitle').textContent='改网络模式';\n"
	...
	"+'请在上级路由的客户端列表中按 MAC 查找：'+esc((INFO&&INFO.mac)||'')+'</div>'\n"
	"+'<div class=w>本机<b>不再发地址</b>；未取得租约时退回当前地址</div>';\n"
	"if(m!='client')h+='<div class=w>本页面将断开，需以 http://'+esc(ip)+'/ 重新打开</div>';\n"
	"h+=sv?'<div class=w><b>保存到闪存</b>：下次开机就用这一套。地址填错时断电也无法恢复</div>'\n"
	":'<div class=w>不保存：只管本次开机，下次开机回到 <b>'+esc(bootdesc())+'</b></div>';\n"
	"if(INFO&&INFO.net&&INFO.net.ack&&m!='server')h+='<div class=w><b>本机用的是设备发的地址</b>'\n"
	"+'：本机这个地址马上就没人续租了，'+(m=='static'?'需手动把本机配成 '\n"
	"+esc(ip.replace(/\\.\\d+$/,'.x'))+' 的一个地址':'请改用上级路由那个网络里的机器')+'</div>';\n"
	"$('#abody').innerHTML=h;\n"
	"var y=$('#yes');y.hidden=false;y.textContent='应用';\n"
	"YES=function(){donetmode(m,ip,mk,sv)};$('#mask').setAttribute('data-on','')}\n"
	"function donetmode(m,ip,mk,sv){\n"
	"get('/netmode?mode='+m+(m=='client'?'':'&ip='+encodeURIComponent(ip)\n"
	"+'&mask='+encodeURIComponent(mk))+'&save='+(sv?1:0),function(st,t){\n"
	"if(st!=200||!/^ok /.test((t||'').trim())){\n"
	"$('#nh').textContent='设备没有接受：'+((t||'').trim()||st);return}\n"
	"hbstop();\n"
	"if(m=='client')gone('正在获取地址',\n"
	"'获取成功后设备位于新地址，请在上级路由的客户端列表中按 <b>'\n"
	"+esc((INFO&&INFO.mac)||'')+'</b> 查找。未取得租约时退回原地址。'\n"
	"+(sv?'':'下次开机回到 <b>'+esc(bootdesc())+'</b>。'),'');\n"
	"else gone('设备已移至 '+ip,\n"
	"'本页面连接的是旧地址，不会自动恢复。请以 <b>http://'+esc(ip)+'/</b> 重新打开。'\n"
	"+(sv?'':'下次开机回到 <b>'+esc(bootdesc())+'</b>。'),'http://'+ip+'/')})}\n"
	"/* 设备换地址之后这个页面就没用了：说清楚去哪找它，别装作还连着 */\n"
	"function gone(title,body,url){GONE=1;netpoll(0);\n"
	"$('#offi').innerHTML=ICON_OFF;$('#offm').className='mi info';\n"
	"$('#offt').textContent=title;\n"
	"$('#offb').innerHTML='<div class=w style=\"color:inherit\">'+body+'</div>'\n"
	...
	"var y=$('#yes');y.hidden=false;y.textContent='启动系统';YES=doboot;$('#mask').setAttribute('data-on'"
		",'')}\n"
	"function doboot(){get('/boot',function(){});expect('boot')}\n"
	"/* 一次性的，所以「设置」之后按钮不该还写着「设置」 */\n"
	"function bootonce(){var b=$('#bob');b.disabled=true;\n"
	"get('/bootonce',function(st,t){b.disabled=false;CHK=null;ENV=null;\n"
	"t=(t||'').trim();\n"
	"if(st!=200){$('#boh').textContent='设置失败（'+st+'）';return}\n"
	"/* saveenv 失败时只在内存里生效，断电即失效 —— 按钮不能跟着说「已设置」 */\n"
	"b.textContent=t.indexOf('armed')<0?'未设置':t.indexOf('saving failed')>=0?'未保存':'已设置';\n"
	"$('#boh').textContent=t.indexOf('armed')<0?'设备没有接受：'+t\n"
	":t.indexOf('saving failed')>=0?'已设置，但未能保存到闪存：断电后失效，'+'请在「诊断」中确认 ubootenv 卷'\n"
	":t.indexOf('already')>=0?'此前已设置，下次开机将停在本页面'\n"
	...
	"(INFO.ubi.vols||[]).slice().sort(function(a,b){var x=(fv[b.n]?1:0)-(fv[a.n]?1:0);return"
		" x||(a.n<b.n?-1:a.n>b.n?1:0)}).forEach(function(v){var n=v.u||v.s;\n"
	"h+='<tr><td>'+esc(v.n)+(fv[v.n]?'<span class=tag>出厂数据</span>':'')+'</td><td>'+esc(v.t)+'</td><td"
		" class=n>'+sz(n)+'</td><td class=b><button type=button class=\"pb dlb\""
		" data-v=\"'+esc(v.n)+'\">下载</button></td></tr>'});\n"
	"t.innerHTML=h;dlbind(t)}\n"
	"/* 卷名是设备给的字符串：只进 data 属性，绝不进 JS 代码里 */\n"
	"function dlbind(t){if(t.__b)return;t.__b=1;\n"
	"t.addEventListener('click',function(e){var b=e.target;\n"
	"while(b&&b!=t){if(b.getAttribute&&/(^| )dlb( |$)/.test(b.className||'')){\n"
	"dlvol(b.getAttribute('data-v'));return}b=b.parentNode}})}\n"
	"/* 单独一层，好让预览把真正的下载换掉而不动上面的逻辑 */\n"
	"function dlstart(u,n){var f=$('#dlsink');f.onload=function(){dlrefused(f)};f.src=u}\n"
	"/* 真下载会被浏览器接走，iframe 不触发 load；只有错误页会 —— 那就是信号 */\n"
	"function dlrefused(f){var t='';\n"
	"try{t=f.contentDocument&&f.contentDocument.body?f.contentDocument.body.textContent:''}catch(e){}\n"
	"t=t.replace(/\\s+/g,' ').trim();if(!t)return;\n"
	"DLBAD=1;dlstop('bad');\n"
	"$('.pwhat',$('#dlprog')).textContent='设备拒绝了本次备份';\n"
	"$('#dlh').textContent='设备拒绝了本次备份：'+t}\n"
	"/*\n"
	"* 进度条只能是不确定的那根，这不是偷懒。net/tcp.c 里只有一个\n"
	"* static struct tcp_stream，旧连接没进 CLOSED 之前新的 SYN 直接被拒 —— 而下载\n"
	"* 那条连接从浏览器点「保存」一直开到文件传完。所以这期间 /dumpinfo 根本连不上\n"
	"* 设备，字节数是问不到的。\n"
	"*\n"
	"* 设备侧照样在数（dump_sent），也照样在 /dumpinfo 里报，只是送不出去；哪天\n"
	"* tcp.c 长出第二条流，这里改回轮询即可。在那之前把话说清楚，比摆一个永远不动\n"
	"* 的百分比强。\n"
	"*/\n"
	"function sink(u,what,n){silent();DLBAD=0;\n"
	"var pg=$('#dlprog');pg.hidden=false;pg.className='prog';\n"
	"$('.pbar',pg).className='pbar ind';$('.pbar',pg).style.width='100%';\n"
	"$('.pwhat',pg).textContent='正在读取并传送 '+what;\n"
	"$('.pct',pg).textContent=n===null?'到片尾':sz(n);\n"
	"$('#dlwhy').hidden=false;\n"
	"$('#dlh').textContent='正在读取并传送 '+what+'（'+(n===null?'到片尾':sz(n))+'）…';\n"
	"get('/dumpinfo',function(st,t){var r=null;try{r=JSON.parse(t)}catch(e){}\n"
	"DLSEQ=(r&&r.seq)||0;dlstart(u,n);dlpoll(0)})}\n"
	"/*\n"
	"* 传输期间这几次问必然打不通（见 sink() 上面那段），所以这里等的只有一件事：\n"
	"* 连接关掉之后 /dumpinfo 的 seq 变了 —— 那说明读完了，crc32 也就有了。\n"
	"*/\n"
	"function dlpoll(k){if(DLBAD)return;\n"
	"get('/dumpinfo',function(st,t){if(DLBAD)return;var r=null;try{r=JSON.parse(t)}catch(e){}\n"
	"if(st==200&&r&&r.seq&&r.seq!=DLSEQ){DLSEQ=r.seq;\n"
	"dlrow(r);return}\n"
	"if(k<120)setTimeout(function(){dlpoll(k+1)},2000);\n"
	"else{dlstop('bad');\n"
	"$('#dlh').textContent='未收到设备回报，本次备份可能未完成，请查看「诊断」中的串口日志'}})}\n"
	"function dlstop(cls){var pg=$('#dlprog');pg.className='prog'+(cls?' '+cls:'');\n"
	"$('#dlwhy').hidden=true;\n"
	"$('.pbar',pg).className='pbar';$('.pbar',pg).style.width='100%';\n"
	"$('.pct',pg).textContent=''}\n"
	...
	"$('#dumpoff').value='0x0';$('#dumplen').value='';dumpraw()}\n"
	"function dumpraw(){var f=INFO&&INFO.flash,o=hex($('#dumpoff').value),lv=$('#dumplen').value.trim"
		"(),l=lv?hex(lv):null;\n"
	"if(o<0){$('#dlh').textContent='起始偏移不是有效的十六进制数';return}\n"
	"if(l!==null&&l<0){$('#dlh').textContent='长度不是有效的十六进制数';return}\n"
	...
	"function envrows(){if(!ENV)return[];var q=$('#envq').value.trim().toLowerCase(),k=$('#envkey').c"
		"hecked;\n"
	"return ENV.env.filter(function(e){if(k&&!ENVKEY.test(e.k))return false;\n"
	"if(!q)return true;return e.k.toLowerCase().indexOf(q)>=0||String(e.v).toLowerCase().indexOf(q)>="
		"0})}\n"
	"function envfill(){var t=$('#envt'),rows=envrows(),h='';\n"
	"if(!ENV)return;\n"
	"rows.forEach(function(e){h+='<tr><td>'+esc(e.k)+'</td><td class=envv>'+esc(e.v)+'</td></tr>'});\n"
	"t.innerHTML=h||'<tr><td class=empty>没有匹配的变量</td></tr>';\n"
	"$('#envh').textContent=rows.length+' / '+ENV.env.length+' 项'+(ENV.cut?' · 已截断，完整内容见串口':'')}\n"
	"/*\n"
	"* bootmenu_<n>=<标题>=<命令>，标题取到第一个 = 为止（U-Boot 自己就是\n"
	"* 这么切的）。标题里可能裹着 ANSI 颜色，串口上红的那几条是危险项，\n"
	"* 这里照样标红。\n"
	"*\n"
	"* 序号列给的是串口菜单上按的那个键，不是变量名里的 n：U-Boot 的\n"
	"* bootmenu 只有一位快捷键，1~9 之后接 a~z，0 留给 Exit，所以\n"
	"* bootmenu_0 在屏幕上是「1.」、bootmenu_9 是「a.」。照变量名从 0\n"
	"* 标起会跟用户眼前的菜单差一位。\n"
	"*/\n"
	"function bmkey(i){return i<9?String(i+1):i<35?String.fromCharCode(88+i):'?'}\n"
	"function bmfill(){var t=$('#bmenu'),m={},h='',d;\n"
	"if(!ENV){t.innerHTML='<tr><td>正在读取…</td></tr>';return}\n"
	"ENV.env.forEach(function(e){var g=/^bootmenu_(\\d+)$/.exec(e.k);\n"
	"if(g)m[+g[1]]=e.v;if(e.k=='bootmenu_delay')d=e.v});\n"
	"Object.keys(m).map(Number).sort(function(a,b){return a-b})\n"
	".forEach(function(i){var v=m[i],p=v.indexOf('='),\n"
	"ti=p<0?v:v.slice(0,p),cm=p<0?'':v.slice(p+1),\n"
	"hot=/\\x1b\\[[0-9;]*3[147]m/.test(ti);\n"
	"ti=ti.replace(/\\x1b\\[[0-9;?]*[A-Za-z]/g,'');\n"
	"h+='<tr><td class=n>'+bmkey(i)+'.</td><td'+(hot?' class=hot':'')+'>'+esc(ti)\n"
	"+'</td><td class=cmd>'+esc(cm)+'</td></tr>'});\n"
	"t.innerHTML=h||'<tr><td class=empty>没有 bootmenu_* 条目</td></tr>';\n"
	"$('#bmh').textContent=h?'设备启动时的菜单，序号即串口上按的键'+(d?'；'+d+' 秒内无按键则执行 bootcmd':'')+'。红色条目会写入闪存'\n"
	":'环境中没有引导菜单：串口不会停顿，直接执行 bootcmd'}\n"
	"function envcopy(b){copy(b,envrows().map(function(e){return e.k+'='+e.v}).join('\\n'))}\n"
	"function askenvdef(){$('#atitle').textContent='环境变量';\n"
	"$('#abody').innerHTML='<div class=r><span>动作</span><span class=v>env default -a \\u0026\\u0026"
		" saveenv</span></div>'+\n"
	"'<div class=w>自定义的 bootcmd、引导菜单与网络设置将恢复为本版 U-Boot 的默认值</div>'+\n"
	"'<div class=w>出厂 MAC、固件与用户配置不受影响；恢复后需重启生效</div>';\n"
	"var y=$('#yes');y.hidden=false;y.textContent='恢复默认';YES=envdef;$('#mask').setAttribute('data-on'"
		",'')}\n"
	"/* 自己的位置：#envh 归 envfill() 管，紧接着的 getenv() 会把它写掉 */\n"
	"function envdef(){$('#envdh').textContent='正在恢复…';\n"
	"get('/envreset',function(st,t){if(st==200){CHK=null;$('#envdh').textContent=t&&t.indexOf('saved'"
		")>=0?'已恢复并保存，重启后生效':'已恢复，但保存失败（UBI 未挂载），改动仅存于内存';getenv()}\n"
	"else $('#envdh').textContent='恢复失败（'+st+'）'})}\n"
	"/*\n"
	"* 「我为什么连不上」这个问题，页面能回答的就这些：地址是什么、地址是不是\n"
	"* 设备发的、哪个口亮着。链路状态与速率在 U-Boot 的通用层拿不到，所以端口\n"
	"* 那几行是直接读 PHY 寄存器来的；读不到的口不列。\n"
	"*/\n"
	"function netfill(){var t=$('#net'),n=(NET&&NET.net)||(INFO&&INFO.net),\n"
	"ports=(NET&&NET.ports)||(INFO&&INFO.ports)||[],h='',m;\n"
	"if(!n){t.innerHTML='<tr><td class=empty>读不到网络信息</td></tr>';return}\n"
	"m=n.mode||'server';\n"
	"h+='<tr><td>模式</td><td>'+(MODES[m]||m)+(n.ram?'<span class=tag>未保存</span>':'')+'</td></tr>';\n"
	"h+='<tr><td>地址</td><td class=mono>'+esc(n.ip)\n"
	"+(n.mask&&n.mask!='0.0.0.0'?' / '+esc(n.mask):'')+'</td></tr>';\n"
	"if(n.gw&&n.gw!='0.0.0.0')h+='<tr><td>网关</td><td class=mono>'+esc(n.gw)+'</td></tr>';\n"
	...
	":n.offer?'已发出 '+n.offer+' 次地址，均未被接受；'\n"
	"+'当前主机用的是手动配置的 IP。'\n"
	":'未发出过地址；当前主机用的是手动配置的 IP，更换主机后需重新配置。';\n"
	"/* 端口那几个数是设备给的，先当数字看待再拼进 HTML */\n"
	"ports.forEach(function(p){var pn=p.p|0,sp=p.speed|0,fd=p.fd|0;\n"
	"h+='<tr><td><span class=\"dot s'+(p.link?0:1)+'\"></span>端口 '+pn+'</td>'\n"
	"+'<td'+(p.link?'':' class=s1')+'>'+(p.link?\n"
	"(sp?sp>=1000?(sp/1000)+' Gb/s':sp+' Mb/s':'已连接')\n"
	"+(sp?fd?' 全双工':' 半双工':''):'未连接')+'</td></tr>'});\n"
	"t.innerHTML=h;\n"
	"$('#portn').hidden=!ports.length;\n"
	"/* 表单只在第一次照着设备填，之后是用户在编辑它，三秒一次的轮询别去动 */\n"
	"if(!NETINIT){NETINIT=1;$('#amode').value=m;\n"
	"if(!$('#nip').value)$('#nip').value=n.ip||'';\n"
	"if(n.mask&&n.mask!='0.0.0.0')$('#nmask').value=n.mask;\n"
	"amodesw()}\n"
	"else bootline()}\n"
	"/*\n"
	"* 卷占用条：每段的宽度是那个卷的预留容量，按卷 ID 排。UBI 把 LEB 映射到\n"
	"* 任意 PEB，还会因磨损均衡把它们搬走，所以这张图说的是「分掉了多少」，\n"
	"* 不是「在哪儿」—— 注释写在图下面那行字里，免得有人当成物理布局。\n"
	"*\n"
	"* 斜纹的两个卷是写入固件时先删掉再重建的，它们的容量算在可写空间里；\n"
	"* 这正是体检「可写空间」那一行的加法。\n"
	"*/\n"
	"var VCOL={fip:'#5e5ce6',fit:'#0a84ff',rootfs_data:'#30d158',\n"
	"ubootenv:'#8e8e93',ubootenv2:'#8e8e93',ri:'#ff9f0a',bosa:'#af52de'};\n"
	"var VPOOL=['#00b4a0','#a2845e','#ff453a','#64748b'];\n"
	"var VRECL={fit:1,rootfs_data:1};\n"
	"function ubibar(){var b=$('#vbar'),g=$('#vleg'),ub=INFO&&INFO.ubi,\n"
	"tot,vols,sum=0,room=0,h='',l='',av,rest;\n"
	"if(!ub||!ub.pebs||!ub.leb){b.hidden=true;g.hidden=true;$('#ubih').textContent='';return}\n"
	"tot=ub.leb*ub.pebs;\n"
	"vols=(ub.vols||[]).slice().sort(function(x,y){return (x.i|0)-(y.i|0)});\n"
	"vols.forEach(function(v,k){var c=VCOL[v.n]||VPOOL[k%VPOOL.length],\n"
	"r=VRECL[v.n]?' re':'',z=v.s|0;\n"
	"sum+=z;if(VRECL[v.n])room+=z;\n"
	"h+='<i class=\"'+r+'\" style=\"flex:'+z+' 0 0;background:'+c\n"
	"+'\" title=\"'+esc(v.n)+' '+sz(z)+'\"></i>';\n"
	"l+='<span><em class=\"'+r+'\" style=\"background:'+c+'\"></em>'+esc(v.n)+' '+sz(z)\n"
	"+(VRECL[v.n]?' · 写入时腾出':'')+'</span>'});\n"
	"rest=tot-sum;if(rest<0)rest=0;\n"
	"/* 老设备的 /info 没有 avail，那就并成一段，别假装分得清 */\n"
	"av=ub.avail==null?null:(ub.avail|0)*ub.leb;\n"
	"if(av==null||av>rest){\n"
	"h+='<i class=free style=\"flex:'+rest+' 0 0\" title=\"空闲与预留 '+sz(rest)+'\"></i>';\n"
	"l+='<span><em class=free></em>空闲与 UBI 预留 '+sz(rest)+'</span>'}\n"
	"else{room+=av;\n"
	"h+='<i class=free style=\"flex:'+av+' 0 0\" title=\"空闲 '+sz(av)+'\"></i>'\n"
	"+'<i class=rsv style=\"flex:'+(rest-av)+' 0 0\" title=\"UBI 预留 '+sz(rest-av)+'\"></i>';\n"
	"l+='<span><em class=free></em>空闲 '+sz(av)+'</span>'\n"
	"+'<span><em style=\"background:var(--c3)\"></em>UBI 预留 '+sz(rest-av)+'</span>'}\n"
	"b.innerHTML=h;g.innerHTML=l;b.hidden=false;g.hidden=false;\n"
	"$('#ubih').textContent='逻辑擦除块 '+sz(ub.leb)+' × '+ub.pebs\n"
	"+'。宽度为各卷的预留容量，按卷 ID 排列；UBI 卷在闪存中并不连续，此图不表示物理位置'\n"
	...
	"(INFO.ubi.vols||[]).slice().sort(function(a,b){return"
	...
	"CHK=r.items;var h='',c=[0,0,0],g='';r.items.forEach(function(i){var"
		...
		" s'+s+'\"></span>'+esc(i.n)+'</td><td class=\"s'+s+'\">'+esc(i.v)+'</td></tr>'});t.innerHTML=h;$('#"
		"chkh').textContent=c[2]?c[2]+' 项异常 · '+c[1]+' 项注意 · '+c[0]+' 项正常':c[1]?c[1]+' 项注意 · '+c[0]+'"
		" 项正常':'全部 '+c[0]+' 项正常'})}\n"
	"/*\n"
	"* 跟随：/log?from=N 只回新字节，正文第一行是新的偏移。页面拿到的是解码\n"
	"* 后的字符串、数不出字节数，所以偏移得由设备说。\n"
	"*/\n"
	"/* 关一次就换一代：2 秒窗里关了又开，旧链的响应回来时对不上代数，就此断掉 */\n"
	...
	"if(i>0){var o=parseInt(t.slice(0,i),10);\n"
	"if(o>=LOGN){LOGN=o;\n"
	"var add=t.slice(i+1).replace(/\\x1b\\[[0-9;?]*[A-Za-z]/g,'').replace(/\\r/g,'');\n"
	"if(add){var l=$('#log');l.textContent+=add;l.scrollTop=l.scrollHeight}}}\n"
	"LOGT=setTimeout(logtick,2000)})}\n"
	"/*\n"
	"* 全片扫描：设备一段一段读，一次答一段。中间页面是活的，所以进度条能走、\n"
	"* 心跳还在，随时可以停。累计的数在这边加，设备那边不留状态。\n"
	"*/\n"
	"var SCAN=null;\n"
	"function scanrun(){var b=$('#scanb'),pg=$('#scanprog');\n"
	"if(SCAN){SCAN.stop=1;SCAN=null;b.textContent='重新扫描';\n"
	"pg.className='prog bad';$('.pwhat',pg).textContent='已停止';\n"
	"$('#scanh').textContent='已停止，结果仅覆盖已扫描的部分';return}\n"
	"SCAN={off:0,size:0,blk:0,bad:0,ecc:0,fail:0,badl:[],faill:[],\n"
	"t0:Date.now(),rt:Date.now(),rn:0,rate:0,stop:0};\n"
	"b.textContent='停止';pg.hidden=false;pg.className='prog';\n"
	"$('.pbar',pg).className='pbar';$('.pbar',pg).style.width='0';\n"
	"$('.pwhat',pg).textContent='正在扫描';$('.pct',pg).textContent='';\n"
	"$('#scanh').textContent='';scanstep()}\n"
	"function scanstep(){var s=SCAN;if(!s||s.stop)return;silent();\n"
	"get('/scan?off=0x'+s.off.toString(16),function(st,t){\n"
	"/* 认自己这一轮：停掉后立刻重扫，旧响应不能往新的那份里加 */\n"
	"if(SCAN!==s||s.stop)return;\n"
	"var r=null;try{r=JSON.parse(t)}catch(e){}\n"
	"if(st!=200||!r||r.err!==undefined){\n"
	"SCAN=null;$('#scanb').textContent='重新扫描';\n"
	"var pg=$('#scanprog');pg.className='prog bad';\n"
	"$('.pwhat',pg).textContent=(r&&r.err)||('扫描没能进行（'+st+'）');\n"
	"return}\n"
	"s.size=r.size;s.blk=r.blk;s.off=r.off;\n"
	"s.bad+=r.bad;s.ecc+=r.ecc;s.fail+=r.fail;\n"
	"(r.badlist||[]).forEach(function(x){if(s.badl.length<32)s.badl.push(x)});\n"
	"(r.faillist||[]).forEach(function(x){if(s.faill.length<32)s.faill.push(x)});\n"
	"scanshow(s,r.done);\n"
	"if(r.done){SCAN=null;$('#scanb').textContent='重新扫描';return}\n"
	"scanstep()})}\n"
	"function hx(n){return '0x'+n.toString(16)}\n"
	"function scanshow(s,done){var pg=$('#scanprog'),p=s.size?s.off/s.size:0,\n"
	"now=Date.now(),dt=(now-s.rt)/1000,h='';\n"
	"if(dt>=1.5){s.rate=(s.off-s.rn)/dt;s.rt=now;s.rn=s.off}\n"
	"$('.pbar',pg).style.width=(p*100)+'%';\n"
	"$('.pct',pg).textContent=sz(s.off)+' / '+sz(s.size)+' · '\n"
	"+(p*100).toFixed(0)+'%'\n"
	"+(!done&&s.rate>0?' · '+spd(s.rate)+' · 剩余 '+dur((s.size-s.off)/s.rate):'');\n"
	"if(done){pg.className='prog'+(s.fail?' bad':' ok');\n"
	"$('.pwhat',pg).textContent=s.fail?'扫描完成，存在无法读出的页':'扫描完成'}\n"
	"h+='<tr><td>已扫描</td><td>'+sz(s.off)+(s.size?' / '+sz(s.size):'')\n"
	"+'（擦除块 '+sz(s.blk||0)+'）</td></tr>';\n"
	"h+='<tr><td><span class=\"dot s'+(s.bad?1:0)+'\"></span>坏块</td>'\n"
	"+'<td class=s'+(s.bad?1:0)+'>'+(s.bad?s.bad+' 个：'+s.badl.map(hx).join('、')\n"
	"+(s.bad>s.badl.length?' …':''):'无')+'</td></tr>';\n"
	"h+='<tr><td><span class=\"dot s'+(s.ecc?1:0)+'\"></span>ECC 纠错</td>'\n"
	"+'<td class=s'+(s.ecc?1:0)+'>'+(s.ecc?s.ecc+' 页在读取时被纠正。'\n"
	"+'少量属正常；成片出现表明颗粒退化，应尽快备份'\n"
	":'无')+'</td></tr>';\n"
	"h+='<tr><td><span class=\"dot s'+(s.fail?2:0)+'\"></span>读失败</td>'\n"
	"+'<td class=s'+(s.fail?2:0)+'>'+(s.fail?s.fail+' 页：'+s.faill.map(hx).join('、')\n"
	"+(s.fail>s.faill.length?' …':'')+'。ECC 无法纠正，这些位置的数据已丢失'\n"
	":'无')+'</td></tr>';\n"
	"$('#scan').innerHTML=h;\n"
	...
	"function copy(b,t){var ok=false;try{var ta=document.createElement('textarea');ta.value=t;ta.styl"
		...
		"clipboard.writeText(t);var o=b.textContent;b.textContent='已复制';setTimeout(function(){b.textConte"
		"nt=o},1500)}\n"
	"function diag(b){var s=['Airoha Web U-Boot '+(INFO&&INFO.web||'')];if(INFO){$$('#dev"
		" tr').forEach(function(r){s.push(r.cells[0].textContent+':"
		" '+r.cells[1].textContent)});if(INFO.ubi)(INFO.ubi.vols||[]).forEach(function(v){s.push('vol"
		" '+v.n+' '+v.t+' '+v.s+' used '+v.u)});else s.push('UBI:"
		" none')}if(CHK)CHK.forEach(function(i){s.push(['OK  ','WARN','FAIL'][i.s]+' '+i.n+':"
		" '+i.v)});else s.push(T('(未运行健康检查)'));copy(b,s.join('\\n'))}\n"
	"/* 复制那份是给聊天窗口用的；这份是给附件用的，把日志和 env 也带上 */\n"
	"function diagtext(){var s=['Airoha Web U-Boot '+(INFO&&INFO.web||'')];\n"
	"s.push(T('导出于 ')+new Date().toLocaleString());s.push('');\n"
	"s.push(T('== 设备 =='));\n"
	"if(INFO){$$('#dev tr').forEach(function(r){s.push(r.cells[0].textContent+':"
		" '+r.cells[1].textContent)});\n"
	"if(INFO.ubi)(INFO.ubi.vols||[]).forEach(function(v){s.push('vol '+v.n+' '+v.t+' '+v.s+' used"
		" '+v.u)});else s.push('UBI: none')}\n"
	"else s.push(T('(/info 读取失败)'));\n"
	"s.push('');s.push(T('== 网络 =='));\n"
	"s.push($('#net').textContent.replace(/\\s+/g,' ').trim()||T('(未读取)'));\n"
	"s.push('');s.push(T('== 快速检查 =='));\n"
	"if(CHK)CHK.forEach(function(i){s.push(['OK  ','WARN','FAIL'][i.s]+' '+i.n+': '+i.v)});\n"
	"else s.push(T('(未运行)'));\n"
	"s.push('');s.push(T('== 环境变量 =='));\n"
	"if(ENV)ENV.env.forEach(function(e){s.push(e.k+'='+e.v)});\n"
	"else s.push(T('(未读取)'));\n"
	"s.push('');s.push(T('== 全片扫描 =='));\n"
	"s.push($('#scan .empty')?T('(未扫描)')\n"
	":$('#scan').textContent.replace(/\\s+/g,' ').trim());\n"
	"s.push('');s.push(T('== 串口日志 =='));\n"
	"s.push($('#log').textContent);\n"
	"return s.join('\\n')}\n"
	"/* 要等 /log 和 /env 回来才拼得全，所以按回调串起来 */\n"
	"function diagfile(b){var o=b.textContent;b.disabled=true;\n"
	"b.textContent='正在收集…';\n"
	"var fin=function(){b.disabled=false;b.textContent=o;\n"
	"var n=(INFO&&INFO.model||'device').replace(/[^A-Za-z0-9_.-]+/g,'-').toLowerCase();\n"
	"save(n+'-diag.txt',diagtext())};\n"
	"var step2=function(){if(ENV)return fin();\n"
	...
	"document.body.removeChild(a);setTimeout(function(){URL.revokeObjectURL(u)},1000)}catch(e){}}\n"
	"/* 写入那几条已经由设备逐行报出来了，p7 只剩试跑走 */\n"
	"function plan(){return ['将固件载入内存并直接引导',\n"
	"'<b>不写入闪存</b>；引导失败断电即恢复原系统']}\n"
	"function ask(){var p=pane(),rows=files(p),W=[],E=[],h='',y=$('#yes');\n"
	"/* 设备详情里唯一能敲回车的是 IP 与掩码：回车就是「应用」，不是弹写入确认 */\n"
	"if(p.id=='p5'){applyaddr();return false}\n"
	"if(p.id=='p10'||p.id=='p11'||p.id=='p12')return false;\n"
	"YES=null;y.textContent='仍要写入';\n"
	"var tb0=$('[name=tryboot]',p);\n"
	"if(tb0&&tb0.checked){if(!rows.some(function(r){return r.k=='firmware'}))\n"
	"E.push('试跑需要选择一个恢复固件');\n"
	"else if(rows.length>1)E.push('试跑仅接收固件本身，其他文件不会被写入');\n"
	"else{W.push('固件仅载入内存，不写入闪存；引导失败断电即恢复原系统');\n"
	"/* 文件名不是凭据，但选错的人多半连名字都没换过 */\n"
	"if(!/recovery/i.test(rows[0].f.name))W.push('这个文件名不像 initramfs 恢复固件。sysupgrade 固件的根文件系统在闪存的 fit"
		" 卷里，试运行不写闪存也就用不到它，内核多半起不来');}\n"
	"y.textContent='启动它'}\n"
	...
	"if(p.id=='p4'&&rows.length){var o=hex($('[name=stockoff]',p).value),f=INFO&&INFO.flash;if(o<0)E."
		...
		" 及其后全部内容'));W.push('写入随上传同步进行，中断将使闪存处于不一致状态，须重传至成功后方可重启');var"
		" wp2=$('[name=wipe]',p),tail=f?f.size-o-rows[0].f.size:0;if(f&&tail>0)W.push(wp2&&wp2.checked?'镜"
		"像之后剩余的 '+sz(tail)+' 将被擦成空白':'镜像之后剩余的 '+sz(tail)+'"
		" 保留原有内容不动');if(!o)W.push('写入后本页面不再可用，重新迁移需经串口')}}\n"
	...
	"* 点下去之前就看见 —— 出厂卷早就有精确尺寸门，这两个一直没有。\n"
	"*/\n"
	"var bl2p=null,fipv=null,\n"
	"rebuild=$('[name=format]',p)&&$('[name=format]',p).checked;\n"
	"(INFO&&INFO.parts||[]).forEach(function(x){if(x.n=='bl2')bl2p=x});\n"
	"(INFO&&INFO.ubi&&INFO.ubi.vols||[]).forEach(function(x){if(x.n=='fip')fipv=x});\n"
	"/* 现存的卷按它自己的预留算；要重建、或压根还没有 fip，则按\n"
	"ubi_write_fip 的创建尺寸 0x100000 —— 与设备侧同一个判据 */\n"
	"var fipcap=fipv&&!rebuild?fipv.s:1048576;\n"
	"rows.forEach(function(r){\n"
	"if(r.k=='bl2'&&bl2p&&r.f.size>bl2p.s-2048)\n"
	...
	"if(p.id=='p1'||(p.id=='p2'&&rows.some(function(r){return r.k=='firmware'})))W.push('rootfs_data"
		" 将被清空');\n"
	"var utot=0;rows.forEach(function(r){utot+=r.f.size});\n"
	...
	"SENT={p:p,rows:rows,btxt:btn.textContent};\n"
	"WR={n:0,off:0,k:0,txt:'',rows:[],ver:null,secs:0,left:0,\n"
	"rt:Date.now(),rn:0,rate:0,fin:0};\n"
	"clearInterval(WRT);WRT=setInterval(wrclock,1000);\n"
	"RT=Date.now();RN=0;RATE=0;\n"
	"HBOFF=1;busy(1);btn.disabled=true;btn.textContent=p.id=='p4'?'写入中…':'上传中…';prog.hidden=false;pro"
		"g.className='prog';bar.className='pbar';bar.style.width='0';\n"
	...
	"bar.style.width=(done/tot*100)+'%';$('.pwhat',prog).textContent=(p.id=='p4'?'正在写入 ':'正在上传"
		" ')+cur.l+(rows.length>1?'（'+(ci+1)+'/'+rows.length+'）':'')+' ·"
		" '+sz(Math.min(done-acc,cur.f.size))+' / '+sz(cur.f.size);\n"
	"/* 速率按最近 1.5 秒算，瞬时值抖得没法看 */\n"
	"var now=Date.now(),dt=(now-RT)/1000;\n"
	"if(dt>=1.5){RATE=(done-RN)/dt;RT=now;RN=done}\n"
	"$('.pct',prog).textContent=sz(done)+' / '+sz(tot)+' · '+(done/tot*100).toFixed(0)+'%'+(RATE>0?'"
		" · '+spd(RATE)+' · 剩余 '+dur((tot-done)/RATE):'')};\n"
	"x.upload.onload=function(){bar.className='pbar ind';var"
		" wz=$('[name=wipe]',p),tb=$('[name=tryboot]',p),tbo=!!(tb&&tb.checked);$('.pwhat',prog).textCont"
		"ent=p.id=='p4'?(wz&&wz.checked?'传输完成，设备正在写入最后一块并擦净尾部…':'传输完成，设备正在写入最后一块…'):tbo?'已载入内存，即将启动':'上传完"
		"成，设备开始写入闪存';\n"
	...
	"/* 刷回原厂走裸端点：镜像边收边写、不进内存，所以没有大小上限；\n"
	"偏移在请求行里，设备收到第一个包就知道该写到哪 */\n"
	"if(p.id=='p4'&&rows.length){var so=hex($('[name=stockoff]',p).value);var wp=$('[name=wipe]',p);\n"
	"x.open('POST','/stock?off=0x'+(so<0?0:so).toString(16)+(wp&&wp.checked?'&wipe=1':''));x.send(row"
		"s[0].f);return}\n"
	"x.open('POST','/');x.send(fd)}\n"
	"function fail(m,st){var p=SENT.p,prog=$('.prog',p),btn=$('button[type=submit]',p);clearInterval("
		"WRT);WRT=null;clearTimeout(WRP);WRP=null;if(WR)WR.fin=1;HBOFF=0;busy(0);prog.className='prog"
		" bad';$('.pbar',prog).className='pbar';$('.pbar',prog).style.width='100%';$('.pwhat',prog).textC"
		"ontent=m;$('.pct',prog).textContent='';btn.disabled=false;btn.textContent=SENT.btxt;\n"
	"/* 流式写到一半断了，闪存就是不一致的；这条提示不能随手一划就没 */\n"
	"if(p.id=='p4'&&(st===0||st>=500)){STUCK=st>=500?'<b>写入未完成，闪存已写入一部分。</b>在重新写入成功之前不要重启设备。':'<b>连接中"
		"断，写入可能未完成。</b>在重新写入成功之前不要重启设备。';banner()}}\n"
	"function done(resp){var p=SENT.p;\n"
	"/* 刷回原厂边收边写，拿到 200 时早写完了，去它自己的完成页 */\n"
	"if(p.id=='p4'){HBOFF=0;stdone(resp);return}\n"
	"if(WR&&WR.fin){HBOFF=0;return}\n"
	"/* 试跑不落闪存，一去不回，所以它仍然是先回复后动手那条路 */\n"
	"if($('[name=tryboot]',p)&&$('[name=tryboot]',p).checked){HBOFF=0;\n"
	"clearInterval(WRT);WRT=null;if(WR)WR.fin=1;\n"
	"$('#steps').innerHTML='<li>'+plan().join('</li><li>')+'</li>';\n"
	"$('#ban').setAttribute('hidden','');\n"
	"TRYB=1;busy(0);expect('boot');show('p7');return}\n"
	"/* 200 只是「收下了」；闪存这才开始动，进度得去问 */\n"
	"wrpoll()}\n"
	"/*\n"
	"* 设备把写到哪了记成一小段文本，这里按 /log?from= 那套取新的部分。写一个卷\n"
	"* 的那几秒它不应答 —— 那一步在环境变量的配方里，run_command 一进去就不回来，\n"
	"* 网络栈也就没在跑。取不到不算事，下次再取；每一行都自带绝对值。\n"
	"*/\n"
	"function wrpoll(){if(!WR||WR.fin)return;\n"
	"get('/wr?from='+WR.off,function(st,t){if(!WR||WR.fin)return;\n"
	"var i=st==200&&t?t.indexOf('\\n'):-1;\n"
	"if(i>0){var o=parseInt(t.slice(0,i),10);\n"
	"if(o>=WR.off){WR.off=o;WR.txt+=t.slice(i+1);wrline(WR.txt)}}\n"
	"if(!WR||WR.fin)return;\n"
	"/* 十来分钟还没有下文，那就不是在写了 */\n"
	"if(++WR.k>1200){fail('未收到设备回报，请查看「诊断」中的串口日志',500);return}\n"
	"WRP=setTimeout(wrpoll,700)})}\n"
	"/*\n"
	"* 写入期间连接一直开着，设备边写边把状态一行行发过来，写完才关连接。所以这里\n"
	"* 收到的不是一次性的回复，而是一条流：\n"
	"*\n"
	"*   s <文字> [字节]      步骤开始；带字节数时按上次实测速度倒计时\n"
	"*   v <已完成> <总数>    回读校验的实时进度（那段循环是设备自己的，报得出）\n"
	"*   r <名> <字节> <crc32>  一个部分写完了\n"
	"*   c ok | c bad <说明>  回读校验的结论\n"
	"*   t <字节> <秒>        本次实测写入速度，存下来给下次估算\n"
	"*   done | f <说明>\n"
	"*\n"
	"* 写入本身报不出中间态 —— 那一步在环境变量的配方里，run_command 一进去就不\n"
	"* 回来了；那是为了和串口 TFTP 刷固件走同一条脚本，值这个代价。\n"
	"*/\n"
	"function wrline(t){var i,l;if(!WR||WR.fin)return;\n"
	"while((i=t.indexOf('\\n',WR.n))>=0){l=t.slice(WR.n,i).replace(/\\r$/,'');WR.n=i+1;\n"
	"if(l)wrstep(l)}}\n"
	"function wrstep(l){var p=SENT.p,prog=$('.prog',p),bar=$('.pbar',prog),\n"
	"k=l.charAt(0),a=l.slice(2).split(' '),n,d,tt,now,dt;\n"
	"if(k=='s'){n=+a[a.length-1];WR.left=0;\n"
	"if(n>0){a.pop();WR.left=Math.max(2,Math.round(n/WRSPD))}\n"
	"bar.className='pbar ind';bar.style.width='100%';\n"
	"$('.pwhat',prog).textContent=a.join(' ')+'…';\n"
	"$('.pct',prog).textContent=WR.left?'预计 '+dur(WR.left):'';return}\n"
	"if(k=='v'){d=+a[0];tt=+a[1];now=Date.now();dt=(now-WR.rt)/1000;WR.left=0;\n"
	"bar.className='pbar';bar.style.width=(tt?d/tt*100:0)+'%';\n"
	"if(dt>=1.5){WR.rate=(d-WR.rn)/dt;WR.rt=now;WR.rn=d}\n"
	"$('.pct',prog).textContent=sz(d)+' / '+sz(tt)+' · '+(tt?(d/tt*100).toFixed(0):0)+'%'\n"
	"+(WR.rate>0?' · '+spd(WR.rate)+' · 剩余 '+dur((tt-d)/WR.rate):'');return}\n"
	"if(k=='r'){WR.rows.push({n:a[0],s:+a[1]||0,c:a[2]||''});return}\n"
	"if(k=='c'){if(a[0]=='ok'){WR.ver='通过';return}\n"
	"STUCK='<b>回读校验没通过，闪存里的内容与上传的不一致。</b>重新写入至成功之前不要重启设备。';\n"
	"banner();fail('回读校验没通过：'+a.slice(1).join(' '),500);return}\n"
	"if(k=='t'){n=+a[0];d=+a[1];\n"
	"if(n>0&&d>0){WRSPD=n/d;try{localStorage.setItem('xgwrspd',String(Math.round(WRSPD)))}catch(e){}}\n"
	"WR.secs=d;return}\n"
	"if(k=='f'){fail(l.slice(2),500);return}\n"
	"if(l=='done')wrfin()}\n"
	"/* 写那一步只有估计，倒计时至少让人看出它在动 */\n"
	"function wrclock(){if(!WR||WR.fin||!WR.left||!SENT)return;\n"
	"WR.left--;$('.pct',$('.prog',SENT.p)).textContent=\n"
	"WR.left>0?'预计还需 '+dur(WR.left):'即将完成…'}\n"
	"/* 写完不再自动重启：设备停在恢复页等着，重不重启这一下由人来点 */\n"
	"function wrfin(){var p=SENT.p,prog=$('.prog',p),bar=$('.pbar',prog),\n"
	"btn=$('button[type=submit]',p),h='';\n"
	"if(!WR||WR.fin)return;\n"
	...
	"* 作废掉，切过去时自然会重跑；关掉这个框时 rhide() 那边同样会重读 /info。\n"
	"*/\n"
	"CHK=null;\n"
	"prog.className='prog ok';bar.className='pbar';bar.style.width='100%';\n"
	"$('.pwhat',prog).textContent='写入完成';$('.pct',prog).textContent='';\n"
	"btn.disabled=false;btn.textContent=SENT.btxt;clear(p);\n"
	"WR.rows.forEach(function(r){h+='<div class=r><span>'+esc(r.n)+'</span><span class=v>'\n"
	"+sz(r.s)+(r.c?' · crc32 '+esc(r.c):'')+'</span></div>'});\n"
	"if(WR.ver)h+='<div class=r><span>回读校验</span><span class=v>'+esc(WR.ver)+'</span></div>';\n"
	...
	"function rboot(){$('#rmask').removeAttribute('data-on');doreboot()}\n"
	"/* 设备回的是 \"ok N bytes crc32 xxxxxxxx skipped M\" */\n"
	"function stdone(t){STUCK='';var m=/ok (\\d+) bytes crc32 ([0-9a-f]+) skipped (-?\\d+)(?: wiped"
		" (\\d+))?/.exec(t||''),h='',k,f=INFO&&INFO.flash;\n"
	"if(m){k=+m[3];\n"
	"h='<tr><td>写入</td><td>'+sz(+m[1])+'（'+m[1]+' 字节）</td></tr>'+\n"
	"'<tr><td>crc32</td><td class=mono>'+m[2]+'</td></tr>'+\n"
	"'<tr><td>坏块</td><td>'+(k?'跳过 '+k+' 块，镜像中对应内容未写入':'无')+'</td></tr>'+(m[4]===undefined?'':'<tr><td"
		">擦净尾部</td><td>'+(f?sz(+m[4]*f.erase)+'（'+m[4]+' 块）':m[4]+' 块')+'</td></tr>')}\n"
	"else h='<tr><td>回报</td><td>'+esc((t||'').trim()||'（无）')+'</td></tr>';\n"
	"CHK=null;$('#stres').innerHTML=h;$('#ban').setAttribute('hidden','');hbstop();show('p13')}\n"
	...
	"/*\n"
	"* 英文版：中文原文就是字典的键 —— 页面里那几百处文案一处都不用改，设备回报\n"
	"* 的中文（/check、/wr、各种错误）也一样走这里。切过去时整棵树替换一遍，之后\n"
	"* 由 MutationObserver 接手后插进来的节点。切回中文是按节点上存下的原文还原，\n"
	"* 不做英译中：设备日志里全是英文，反查会把它们也一起换掉。\n"
	"*\n"
	"* I18N 整段匹配，I18P 按片段替换 —— 只有中间夹着数字、容量、卷名的句子才\n"
	"* 需要后者，「无」这类短词放进去会在别的句子里乱咬。\n"
	"*/\n"
	"/* i18n-dict begin */\n"
	"var I18N={\n"
	...
	"\"连接中…\":\"Linking…\",\n"
	"\"将 sysupgrade 固件写入 fit 卷。rootfs_data 将被清空；系统可正常启动时请使用 sysupgrade。\":\"Writes a sysupgrade image"
		" into the fit volume. rootfs_data is erased. While the system still boots, use sysupgrade"
		" instead.\",\n"
	"\"固件\":\"Firmware\",\n"
	"\"选择固件文件\":\"Choose a firmware file\",\n"
	"\"选择文件…\":\"Choose file…\",\n"
	"\"设备停在恢复模式，不会自行引导。写入开始前闪存不会被修改；写完回读校验一遍，再由你决定是否重启。\":\"The device is held in recovery and will not"
		" boot on its own. Nothing is written to flash until the upload finishes; the image is then read"
		" back and verified, and rebooting is your call.\",\n"
	"\"想先不写闪存试一次，去「\":\"To try an image without touching flash, go to \",\n"
	"\"未选择文件\":\"No file selected\",\n"
	"\"上传并刷写\":\"Upload and flash\",\n"
	"\"写入 BL2 与 U-Boot FIP。用于从 tcboot / 原厂布局首次迁移，或升级 U-Boot。\":\"Writes BL2 and the U-Boot FIP. Use it"
		" for the first migration from a tcboot / stock layout, or to upgrade U-Boot.\",\n"
	"\"未选择\":\"None selected\",\n"
	"\"可选，同时写入\":\"Optional, written in the same pass\",\n"
	"\"首次迁移\":\"First migration\",\n"
	"\"重建 UBI\":\"Rebuild UBI\",\n"
	...
	"\"」。\":\".\",\n"
	"\"。\":\".\",\n"
	"\"重建擦除的范围从 0x20000 起，盖住了原厂引导器的后半截\":\"The rebuild erases from 0x20000 on, which covers the back"
		" half of the stock bootloader\",\n"
	"\"（原厂 bootloader 分区是 0x0–0x80000，而本布局的 bl2 只占 0x0–0x20000）。所以只写 U-Boot、不写 BL2 的话，重启时原厂 BL2"
		...
		" serial console gets you out of that. Hence this option demands BL2 and U-Boot together.\",\n"
	"\"仅升级 U-Boot 时不需重建 UBI：只选择 U-Boot 文件，rootfs_data 保留。\":\"Upgrading U-Boot alone needs no rebuild:"
		" pick only the U-Boot file and rootfs_data survives.\",\n"
	"\"把恢复固件载入内存直接引导，\":\"Loads a recovery image into RAM and boots it. \",\n"
	"\"闪存一个字节都不写\":\"Not one byte of flash is written\",\n"
	"\"。起不来断电即恢复原系统。\":\". If it does not come up, power-cycle and the old system is back.\",\n"
	"\"选择恢复固件\":\"Choose a recovery image\",\n"
	"\"要用 initramfs 恢复固件。\":\"Use an initramfs recovery image. \",\n"
	"\"它的根文件系统随镜像一起进内存，不依赖闪存，所以 UBI 是空的、fit 卷没了也照样跑得起来。\":\"Its root filesystem travels into RAM with"
		" the image and needs no flash, so it runs even when UBI is empty or the fit volume is gone. \",\n"
	"\"sysupgrade 固件不适合这里\":\"A sysupgrade image will not do here\",\n"
	"\"：它的根要由 fitblk 从闪存的 fit 卷里读（设备树的 rootdisk 指着那个卷），试跑时内核是新的、根还是闪存里那份旧的，多半起不来。\":\": its root is"
		" read by fitblk from the fit volume in flash (rootdisk in the device tree points there), so a"
		" trial run pairs a new kernel with the old root still in flash, and usually fails to boot.\",\n"
	"\"引导成功后本页面不再可用；要回到恢复页，断电重来即可。闪存没写过一个字节，原系统还在。\":\"Once it boots, this page is gone; power-cycle to"
		" get back to recovery. Flash was never written and the old system is intact.\",\n"
	"\"启动它\":\"Boot it\",\n"
	"\"将镜像写入 flash 指定偏移，边接收边写入、不在内存中暂存，因此没有单独的上传阶段。偏移为 0 时整片写入，当前引导程序与本页面将被覆盖；重新迁移至 OpenWrt 需经"
		" USB-TTL 串口。\":\"Writes an image to a given flash offset. It is written as it arrives rather than"
	...
	"\"镜像\":\"Image\",\n"
	"\"写入偏移\":\"Write offset\",\n"
	"\"十六进制，按擦除块对齐\":\"Hexadecimal, aligned to an erase block\",\n"
	"\"0 为整片，亦可写入单个分区\":\"0 writes the whole chip; a single partition works too\",\n"
	"\"擦净尾部\":\"Erase the tail\",\n"
	"\"镜像之后\":\"The erase blocks after the image, \",\n"
	"\"直至片尾\":\"all the way to the end of the chip\",\n"
	"\"的擦除块一并擦空；不擦则保留原有内容\":\", are erased as well. Leave it off to keep what is there\",\n"
	"\"镜像须来自本机备份，可在「\":\"The image must come from a backup of this board, exported from \",\n"
	"\"」中导出。长度上限为 flash 容量 \":\". Its length is capped at the flash size, \",\n"
	"\"页面不校验镜像内容、机型与偏移是否匹配\":\"This page does not check that the image, the model and the offset match\",\n"
	"\"，写错仅能通过串口恢复。整片写入耗时显著长于固件写入；\":\", and a wrong write can only be undone over serial. A whole-chip"
		" write takes far longer than a firmware write. \",\n"
	"\"写入一旦开始，中断将使闪存处于不一致状态，须重传至成功后方可重启\":\"Once writing starts, an interruption leaves flash"
		" inconsistent; resend until it succeeds before rebooting\",\n"
	"\"。写入期间指示灯\":\". The LEDs \",\n"
	"\"流水\":\"chase\",\n"
	"\"，与其他页面一致；本页写入与接收同时进行，\":\" while writing, as on the other pages. Here writing and receiving"
		" happen at once, so \",\n"
	"\"网线与电源都不能断\":\"neither the cable nor the power may be cut\",\n"
	"\"，进度以本页进度条为准。\":\". Go by the progress bar on this page.\",\n"
	"\"刷写\":\"Flash it\",\n"
	"\"写入 UBI 卷\":\"Write UBI volume\",\n"
	"\"按卷名写入 UBI 卷，卷不存在时按文件长度创建。物理位置由 UBI 层管理，不涉及 bl2 分区。\":\"Writes a UBI volume by name, creating it"
		" at the file's size if it does not exist. UBI decides where it physically lands; the bl2"
		" partition is not involved.\",\n"
	"\"出厂数据\":\"Factory data\",\n"
	"\"文件须来自本机备份，可同时写入\":\"The files must come from a backup of this board; several can go in one pass\",\n"
	"\"任意卷\":\"Any volume\",\n"
	"\"卷名错误将覆盖对应卷的内容\":\"A wrong name overwrites whatever that volume holds\",\n"
	"\"卷名\":\"Volume name\",\n"
	"\"如 fip、fit\":\"e.g. fip, fit\",\n"
	"\"仅限字母、数字与 _ - .\":\"Letters, digits and _ - . only\",\n"
	"\"内容\":\"Contents\",\n"
	"\"卷\":\"Volume\",\n"
	"\"卷内容\":\"Volume contents\",\n"
	"\"写完由你决定是否重启，所以可以连着写好几个卷。\":\"Rebooting is your call once a write finishes, so several volumes can"
		" be written back to back.\",\n"
	"\"读取闪存内容并下载。读取与传输同步进行，不限长度；传输期间设备可能暂停响应。\":\"Reads flash and downloads it. Reading and"
		" transferring run together with no length limit; the device may stop answering while it does.\",\n"
	"\"UBI 卷\":\"Volumes\",\n"
	"\"原始区段\":\"Raw range\",\n"
	"\"正在读取…\":\"Reading…\",\n"
	"\"绕过 UBI 按 flash 偏移读取，偏移与「刷回原厂」一致。\":\"Reads by flash offset, bypassing UBI. The offsets match the"
		" ones on Restore stock.\",\n"
	"\"起始偏移\":\"Start offset\",\n"
	"\"十六进制\":\"Hexadecimal\",\n"
	"\"0 为片首\":\"0 is the start of the chip\",\n"
	"\"长度\":\"Length\",\n"
	"\"留空到片尾\":\"empty = to the end\",\n"
	"\"留空读至片尾\":\"Leave empty to read to the end of the chip\",\n"
	"\"整片读取的是闪存本身，不经过分区表，原厂布局下的 romfile、config 一并包含。\":\"A whole-chip read takes the flash itself, not"
		" the partition table, so on a stock layout romfile and config come along.\",\n"
	"\"文件偏移等于 flash 偏移\":\"Offsets in the file equal flash offsets\",\n"
	"\"，坏块在文件中保留占位，格式与 \":\". Bad blocks keep their place in the file, and the format matches a \",\n"
	"\" 镜像一致：外部 \":\" image: an \",\n"
	"\" 可直接写入，此处导出的镜像亦可用于编程器。写回时坏块跳过而不压缩，其后内容位置不变。\":\" from elsewhere can be written straight back,"
		" and an image exported here also works in a programmer. On the way back bad blocks are skipped"
		" rather than squeezed out, so everything after them stays put.\",\n"
	"\"整片下载\":\"Download whole chip\",\n"
	"\"下载区段\":\"Download range\",\n"
	"\"已完成\":\"Finished\",\n"
	"\"crc32 基于实际传出的字节计算，可与本地文件核对\":\"The crc32 covers the bytes actually sent, so it can be checked"
		" against the file you got\",\n"
	"\"选择卷，或填写偏移与长度\":\"Pick a volume, or fill in an offset and length\",\n"
	"\"传输期间设备不应答：U-Boot 的 TCP 栈同时只有一条连接，而这条连接正被下载占着，所以本页面问不到「传了多少字节」。\":\"The device goes quiet during"
		" the transfer: U-Boot's TCP stack holds one connection at a time and the download owns it, so"
		" this page cannot ask how many bytes have gone out. \",\n"
	"\"字节进度请看浏览器自己的下载栏\":\"Watch the browser's own download bar for that\",\n"
	"\"。传完这里会给出 crc32，可与本地文件核对。\":\". When it finishes, the crc32 appears here to check against the"
		" file.\",\n"
	"\"运行时从设备树、MTD 与 UBI 读取。\":\"Read at runtime from the device tree, MTD and UBI.\",\n"
	"\"硬件\":\"Hardware\",\n"
	"\"网络\":\"Network\",\n"
	"\"端口号为交换机内部顺序，与外壳丝印不一定对应；无响应的端口不列出。\":\"Port numbers follow the switch's internal order, which"
		" need not match the labels on the case. Ports that do not answer are left out.\",\n"
	"\"地址\":\"Address\",\n"
	"\"三种模式互斥，应用后需以新地址重新打开本页面\":\"The three modes are exclusive; after applying, reopen this page at"
		" the new address\",\n"
	"\"模式\":\"Mode\",\n"
	"\"DHCP 服务器 · 本机发地址\":\"DHCP server · this device hands out addresses\",\n"
	"\"静态地址\":\"Static address\",\n"
	"\"DHCP 客户端 · 向上级路由要\":\"DHCP client · ask the upstream router\",\n"
	"\"路由器 IP\":\"Router IP\",\n"
	"\"本机地址，末位固定为 1\":\"This device's address; the last octet is always 1\",\n"
	"\"子网掩码\":\"Subnet mask\",\n"
	"\"保存到闪存\":\"Save to flash\",\n"
	"\"不保存则只在本次开机有效\":\"Without this it only lasts until the next power-up\",\n"
	"\"应用\":\"Apply\",\n"
	"\"读取设备状态：关键位置检查、全片扫描，以及本次上电的控制台输出。\":\"Reads the device's state: a check of the critical spots, a"
		" whole-chip scan, and the console output since this power-up.\",\n"
	"\"快速检查\":\"Quick check\",\n"
	"\"全片扫描\":\"Full scan\",\n"
	"\"串口日志\":\"Serial log\",\n"
	"\"检查 BL2、坏块、UBI 各卷与 U-Boot MAC。读取闪存期间设备暂停响应，耗时数秒。\":\"Checks BL2, bad blocks, the UBI volumes and"
		" the U-Boot MAC. The device stops answering while it reads flash, for a few seconds.\",\n"
	"\"未执行检查\":\"Not checked yet\",\n"
	"\"点「开始检查」读取 BL2、坏块与各卷状态\":\"Press Run check to read BL2, the bad blocks and the state of each"
		" volume\",\n"
	"\"开始检查\":\"Run check\",\n"
	"\"复制诊断信息\":\"Copy diagnostics\",\n"
	"\"下载诊断包\":\"Download diagnostics\",\n"
	"\"未扫描\":\"Not scanned\",\n"
	"\"点「开始扫描」逐页读一遍整片闪存\":\"Press Start scan to read every page of the chip once\",\n"
	"\"逐页读取全片；只读，不改动闪存。「快速检查」只覆盖关键位置，颗粒退化需整片读取才能发现。分段执行，段间页面保持响应；整片耗时数分钟，其间设备响应变慢。\":\"Reads the whole"
		" chip page by page. Read-only; flash is not modified. Quick check only covers the critical"
		" spots, while a decaying NAND die shows up only in a full read. It runs in chunks and the page"
		" stays responsive between them; the whole chip takes minutes and the device answers more slowly"
		" meanwhile.\",\n"
	"\"开始扫描\":\"Start scan\",\n"
	"\"U-Boot 本次上电以来的控制台输出。\":\"U-Boot's console output since this power-up.\",\n"
	"\"未读取\":\"Not read yet\",\n"
	"\"日志缓冲已满，后续输出未记录。重启后重新开始记录。\":\"The log buffer is full and later output was not recorded."
		" Recording restarts after a reboot.\",\n"
	"\"实时跟随\":\"Follow live\",\n"
	"\"读取日志\":\"Read log\",\n"
	"\"复制日志\":\"Copy log\",\n"
	"\"U-Boot 环境变量\":\"U-Boot environment\",\n"
	"\"当前生效的变量，只读。bootcmd 与引导菜单异常是无法启动的常见原因。\":\"The variables in effect, read-only. A broken bootcmd"
		" or boot menu is a common reason a device will not start.\",\n"
	"\"变量\":\"Variables\",\n"
	"\"引导菜单预览\":\"Boot menu preview\",\n"
	"\"恢复默认\":\"Restore defaults\",\n"
	"\"过滤名称或值\":\"Filter by name or value\",\n"
	"\"只看关键项\":\"Key variables only\",\n"
	"\"重新读取\":\"Read again\",\n"
	"\"复制全部\":\"Copy all\",\n"
	"\"设备启动时的菜单，由 bootmenu_* 变量构成。\":\"The menu shown at boot, built from the bootmenu_* variables.\",\n"
	"\"恢复为本版 U-Boot 的默认值\":\"Restore this U-Boot's own defaults\",\n"
	"\"清除自定义的 bootcmd、引导菜单与网络设置并保存\":\"Clears a customised bootcmd, boot menu and network settings, and"
		" saves\",\n"
	"\"出厂 MAC 位于 ri 卷，\":\"The factory MAC lives in the ri volume and is \",\n"
	"\"不受影响\":\"left alone\",\n"
	"\"；固件与用户配置同样不受影响。恢复后需重启生效。UBI 无法挂载时保存失败，改动仅存于内存。\":\"; firmware and user settings are untouched as"
		" well. A reboot is needed for it to take effect. If UBI cannot be mounted the save fails and"
		" the change lives only in RAM.\",\n"
	"\"离开本页面的三种方式，均不改动闪存。\":\"Three ways to leave this page. None of them touches flash.\",\n"
	"\"重启后正常引导，\":\"After a reboot the device boots normally, and \",\n"
	...
	"\" —— 之后这个地址上是系统自己的页面。只有引导失败才回到恢复页，届时本页面自动刷新。\":\" — this address then belongs to the system's own"
		" page. Only a failed boot comes back to recovery, and this page reloads itself when it does.\",\n"
	"\"立即重启\":\"Reboot now\",\n"
	"\"其他启动方式\":\"Other ways to start\",\n"
	"\"直接启动系统\":\"Boot the system now\",\n"
	"\"执行 bootcmd，不经过冷启动；引导失败回到本页面\":\"Runs bootcmd without a cold start; a failed boot returns to this"
		" page\",\n"
	"\"启动系统\":\"Boot system\",\n"
	"\"下次开机进恢复页\":\"Stop in recovery next boot\",\n"
	"\"仅生效一次，进入后自动还原\":\"One shot; it clears itself once you get here\",\n"
	"\"设置\":\"Arm\",\n"
	"\"设置后下次开机停在本页面，再下次开机恢复正常引导。\":\"Once armed, the next power-up stops on this page and the one after"
		" that boots normally again.\",\n"
	"\"U-Boot 内置的 HTTP 恢复服务，页面不依赖任何外部资源。\":\"An HTTP recovery service built into U-Boot. The page loads"
	...
	"\"问题反馈\":\"Issues\",\n"
	"\"基于\":\"Based on\",\n"
	"\"已交给设备启动\":\"Handed over to the device\",\n"
	"\"设备正在从内存引导该固件，闪存未改动。引导成功后本页面不再可用。\":\"The device is booting that image from RAM. Flash was not"
		" modified. Once it boots, this page is gone.\",\n"
	"\"起不来怎么办\":\"If it does not come up\",\n"
	"\"断电\":\"Power off\",\n"
	"\"再上电即回到原有系统，闪存一个字节都没写\":\"Power it back on and the old system is there; not one byte of flash was"
		" written\",\n"
	"\"等待\":\"Wait\",\n"
	"\"引导失败时设备自行回到本页面，届时自动刷新\":\"A failed boot brings the device back to this page, which then reloads"
		" itself\",\n"
	"\"本次动作\":\"What happens\",\n"
	"\"写入完成\":\"Write finished\",\n"
	"\"镜像已全部写入闪存，设备正在重启。本页面可以关闭。\":\"The whole image is in flash and the device is rebooting. You can"
		" close this page.\",\n"
	"\"设备回报\":\"Device reported\",\n"
	"\"crc32 由设备对收到的字节计算，与备份时记录的值一致即说明传输无误。\":\"The device computed the crc32 over the bytes it"
		" received; if it matches the one noted at backup time, the transfer was clean.\",\n"
	"\"重启约需 1–2 分钟。整片写入后本页面所属的 U-Boot 已被覆盖，设备将按镜像中的引导程序启动。\":\"The reboot takes one or two minutes."
		" After a whole-chip write the U-Boot this page belongs to is gone, and the device starts from"
		" the bootloader inside the image.\",\n"
	"\"重新连接\":\"Reconnect\",\n"
	"\"确认写入\":\"Confirm write\",\n"
	"\"取消\":\"Cancel\",\n"
	"\"仍要写入\":\"Write anyway\",\n"
	"\"写入完毕\":\"Write complete\",\n"
	"\"留在恢复页\":\"Stay in recovery\",\n"
	"\"已连接\":\"Online\",\n"
	"\"设备忙…\":\"Busy…\",\n"
	"\"无响应…\":\"Quiet…\",\n"
	"\"已断开\":\"Offline\",\n"
	"\"闪存中没有可挂载的 UBI。\":\"There is no mountable UBI in flash. \",\n"
	"\"首次迁移：在「引导升级」中同时上传 BL2、U-Boot 与固件，并启用「重建 UBI」。\":\"First migration: on Bootloader, upload BL2,"
		" U-Boot and the firmware together and turn on Rebuild UBI.\",\n"
	...
	"\"；引导失败才回到这里并自动刷新。\":\"; only a failed boot comes back here, and the page reloads itself.\",\n"
	"\"设备正在启动系统\":\"The device is booting the system\",\n"
	"\"闪存未改动。引导成功后\":\"Flash was not modified. Once it boots, \",\n"
	"\"；引导失败则回到本页面。\":\"; a failed boot returns to this page.\",\n"
	"\"与设备的连接已断开\":\"The connection to the device is down\",\n"
	...
	"\"闪存上没有可挂载的 UBI，当前 U-Boot 仅存于内存。\":\"There is no mountable UBI in flash and the running U-Boot"
		" lives only in RAM. \",\n"
	"\"重启后回到原有系统，需重新经串口传入 U-Boot 才能再打开本页面。建议先在「引导升级」中完成写入\":\"After a reboot the old system is back,"
		" and U-Boot has to be sent over serial again before this page can be opened. Better to finish"
		" the write on Bootloader first\",\n"
	"\"闪存中没有 U-Boot（fip 卷），当前 U-Boot 仅存于内存。\":\"There is no U-Boot in flash (the fip volume) and the"
		" running one lives only in RAM. \",\n"
	"\"重启后本页面将无法再打开\":\"After a reboot this page can no longer be opened\",\n"
	"\"重启\":\"Reboot\",\n"
	"\"动作\":\"Action\",\n"
	"\"DHCP 服务器\":\"DHCP server\",\n"
	"\"DHCP 客户端\":\"DHCP client\",\n"
	"\"设备的接口地址\":\"The device's interface address\",\n"
	...
	"\"接入已有网络时用这一档：填一个该网段内的空闲地址，本机不再发地址，应用后即可从网内任何一台机器打开本页面。\":\"Use this mode on an existing network:"
		" give it a free address on that subnet. The device stops handing out addresses, and once"
		" applied this page opens from any machine on the network.\",\n"
	"\"地址由上级路由分配，本页面事先不知道是多少，需在上级路由的客户端列表中按 MAC 查找。本机不再发地址。未取得租约时退回当前地址。\":\"The upstream router"
		" assigns the address, which this page cannot know in advance — look it up by MAC in the"
		" router's client list. The device stops handing out addresses. Without a lease it falls back to"
		" the current address.\",\n"
	"\"主机需在同一网段\":\"Your computer must be on the same subnet\",\n"
	"\"填本网段内任一地址即可，如 192.168.1.1\":\"Any address on this subnet will do, e.g. 192.168.1.1\",\n"
	"\"DHCP 服务器 192.168.1.1 / 255.255.255.0（出厂默认）\":\"DHCP server 192.168.1.1 / 255.255.255.0 (factory"
		" default)\",\n"
	"\"。勾上「保存到闪存」才会把当前这一套写进去替换它；不勾就只管本次开机。\":\". Only Save to flash writes the current settings over"
		" it; without it they last until the next power-up.\",\n"
	"\"IP 不是一个合法的地址\":\"That is not a valid IP address\",\n"
	"\"子网掩码不是一个合法的地址\":\"That is not a valid subnet mask\",\n"
	"\"改网络模式\":\"Change network mode\",\n"
	"\"本机\":\"This device\",\n"
	"\"不再发地址\":\"stops handing out addresses\",\n"
	"\"，电脑需手动配置同网段的 IP\":\", so the computer needs a manual IP on the same subnet\",\n"
	"\"地址由上级路由分配，本页面无法预知。\":\"The upstream router assigns the address; this page cannot know it. \",\n"
	"\"；未取得租约时退回当前地址\":\"; without a lease it falls back to the current address\",\n"
	"\"：下次开机就用这一套。地址填错时断电也无法恢复\":\": the next power-up uses these settings. If the address is wrong, a"
		" power-cycle will not undo it\",\n"
	"\"本机用的是设备发的地址\":\"Your computer is using an address this device handed out\",\n"
	"\"请改用上级路由那个网络里的机器\":\"use a machine on the upstream router's network instead\",\n"
	"\"正在获取地址\":\"Getting an address\",\n"
	"\"10 秒后自动跳转。\":\"Jumping there in 10 seconds.\",\n"
	"\"闪存内容不受影响；引导失败回到本页面\":\"Flash is not affected; a failed boot returns to this page\",\n"
	"\"未设置\":\"Not armed\",\n"
	"\"未保存\":\"Not saved\",\n"
	"\"已设置\":\"Armed\",\n"
	"\"已设置，但未能保存到闪存：断电后失效，请在「诊断」中确认 ubootenv 卷\":\"Armed, but it could not be saved to flash: it will"
		" not survive a power-off. Check the ubootenv volume under Diagnostics\",\n"
	"\"此前已设置，下次开机将停在本页面\":\"Already armed; the next power-up stops on this page\",\n"
	"\"已设置。下次开机停在本页面，再下次开机恢复正常引导\":\"Armed. The next power-up stops on this page, and the one after"
		" that boots normally again\",\n"
	"\"读取失败，刷新页面重试\":\"Read failed. Reload the page and try again\",\n"
	"\"UBI 未挂载\":\"UBI is not mounted\",\n"
	"\"首次迁移前闪存仍为原厂内容，建议此时用「原始区段」整片备份\":\"Before the first migration flash still holds the stock content"
		" — this is the moment to back up the whole chip from Raw range\",\n"
	"\"下载\":\"Download\",\n"
	"\"设备拒绝了本次备份\":\"The device refused this backup\",\n"
	"\"到片尾\":\"to the end\",\n"
	"\"未收到设备回报，本次备份可能未完成，请查看「诊断」中的串口日志\":\"No word back from the device — the backup may be incomplete."
		" Check the serial log under Diagnostics\",\n"
	"\"传输完成\":\"Transfer complete\",\n"
	"\"无法读取闪存信息\":\"Cannot read the flash information\",\n"
	"\"起始偏移不是有效的十六进制数\":\"The start offset is not a valid hexadecimal number\",\n"
	...
	"\"环境中没有引导菜单：串口不会停顿，直接执行 bootcmd\":\"The environment has no boot menu: the console will not pause"
		" and bootcmd runs straight away\",\n"
	"\"自定义的 bootcmd、引导菜单与网络设置将恢复为本版 U-Boot 的默认值\":\"A customised bootcmd, boot menu and network"
		" settings go back to this U-Boot's defaults\",\n"
	"\"出厂 MAC、固件与用户配置不受影响；恢复后需重启生效\":\"The factory MAC, the firmware and user settings are untouched; a"
		" reboot is needed for it to take effect\",\n"
	"\"正在恢复…\":\"Restoring…\",\n"
	"\"已恢复并保存，重启后生效\":\"Restored and saved; it takes effect after a reboot\",\n"
	"\"已恢复，但保存失败（UBI 未挂载），改动仅存于内存\":\"Restored, but the save failed (UBI is not mounted), so the change"
		" lives only in RAM\",\n"
	"\"读不到网络信息\":\"Cannot read the network information\",\n"
	"\"网关\":\"Gateway\",\n"
	"\"网卡\":\"Interface\",\n"
	"\"未发出过地址；当前主机用的是手动配置的 IP，更换主机后需重新配置。\":\"No address has been handed out; your computer is on a"
		" manually configured IP, and another machine would need configuring too.\",\n"
	"\"未连接\":\"No link\",\n"
	"\"机型\":\"Model\",\n"
	"\"内存\":\"RAM\",\n"
	"\"闪存\":\"Flash\",\n"
	...
	"\"已用\":\"Used\",\n"
	"\"检查中…\":\"Checking…\",\n"
	"\"正在检查，读取闪存期间设备不响应…\":\"Checking. The device does not answer while it reads flash…\",\n"
	"\"重新检查\":\"Check again\",\n"
	"\"重新扫描\":\"Scan again\",\n"
	"\"已停止\":\"Stopped\",\n"
	"\"已停止，结果仅覆盖已扫描的部分\":\"Stopped; the result only covers what was scanned\",\n"
	"\"停止\":\"Stop\",\n"
	"\"正在扫描\":\"Scanning\",\n"
	"\"扫描完成，存在无法读出的页\":\"Scan finished; some pages could not be read\",\n"
	"\"扫描完成\":\"Scan finished\",\n"
	"\"已扫描\":\"Scanned\",\n"
	"\"坏块\":\"Bad blocks\",\n"
	"\"无\":\"None\",\n"
	"\"ECC 纠错\":\"ECC corrections\",\n"
	"\"读失败\":\"Read failures\",\n"
	"\"扫描完成，见上表\":\"Scan finished; see the table above\",\n"
	"\"扫描完成，全片可读\":\"Scan finished; the whole chip reads\",\n"
	"\"已复制\":\"Copied\",\n"
	"\"(未运行健康检查)\":\"(no health check was run)\",\n"
	"\"导出于 \":\"Exported \",\n"
	"\"== 设备 ==\":\"== Device ==\",\n"
	"\"(/info 读取失败)\":\"(/info could not be read)\",\n"
	"\"== 网络 ==\":\"== Network ==\",\n"
	"\"(未读取)\":\"(not read)\",\n"
	"\"== 快速检查 ==\":\"== Quick check ==\",\n"
	"\"(未运行)\":\"(not run)\",\n"
	...
	"\"== 串口日志 ==\":\"== Serial log ==\",\n"
	"\"正在收集…\":\"Collecting…\",\n"
	"\"将固件载入内存并直接引导\":\"Load the image into RAM and boot it\",\n"
	"\"不写入闪存\":\"Flash is not written\",\n"
	"\"；引导失败断电即恢复原系统\":\"; if it fails to boot, power-cycle and the old system is back\",\n"
	"\"试跑需要选择一个恢复固件\":\"A trial run needs a recovery image\",\n"
	"\"试跑仅接收固件本身，其他文件不会被写入\":\"A trial run takes the image alone; no other file is written\",\n"
	"\"固件仅载入内存，不写入闪存；引导失败断电即恢复原系统\":\"The image only goes into RAM, not flash; if it fails to boot,"
		" power-cycle and the old system is back\",\n"
	"\"这个文件名不像 initramfs 恢复固件。sysupgrade 固件的根文件系统在闪存的 fit 卷里，试运行不写闪存也就用不到它，内核多半起不来\":\"That file name"
		" does not look like an initramfs recovery image. A sysupgrade image keeps its root filesystem"
		" in the fit volume in flash, which a trial run never touches, so the kernel will most likely"
		" not come up\",\n"
	"\"写入偏移不是有效的十六进制数\":\"The write offset is not a valid hexadecimal number\",\n"
	"\"写入随上传同步进行，中断将使闪存处于不一致状态，须重传至成功后方可重启\":\"Writing runs alongside the upload; an interruption"
		" leaves flash inconsistent, so resend until it succeeds before rebooting\",\n"
	"\"写入后本页面不再可用，重新迁移需经串口\":\"After this write the page is gone, and migrating back needs a serial"
		" console\",\n"
	"\"选择了卷内容但未填写卷名\":\"Volume contents were picked but no volume name was given\",\n"
	"\"填写了卷名但未选择卷内容\":\"A volume name was given but no contents were picked\",\n"
	"\"卷名仅限字母、数字与 _ - .\":\"A volume name may only contain letters, digits and _ - .\",\n"
	"\"打开了「重建 UBI」却没有选择 U-Boot 文件：重建会抹掉 fip 卷，没有 U-Boot 设备将无法启动\":\"Rebuild UBI is on but no U-Boot"
		" file was picked: the rebuild wipes the fip volume, and without U-Boot the device will not"
		" start\",\n"
	"\"打开了「重建 UBI」却没有选择 BL2：重建从 0x20000 起擦，盖住了原厂引导器的后半截，只写 U-Boot 的话重启起不来，只能拆串口救\":\"Rebuild UBI is on"
		" but no BL2 was picked: the rebuild erases from 0x20000 on, which covers the back half of the"
		" stock bootloader. Writing U-Boot alone leaves a device that will not boot, and only a serial"
		" console gets you out of that\",\n"
	"\"闪存里没有 U-Boot（fip 卷），本次必须同时上传 U-Boot 文件\":\"There is no U-Boot in flash (the fip volume), so a"
		" U-Boot file has to go in this time\",\n"
	"\"闪存里没有可挂载的 UBI：请打开「重建 UBI」，并同时上传 BL2、U-Boot 与固件\":\"There is no mountable UBI in flash: turn on"
		" Rebuild UBI and upload BL2, U-Boot and the firmware together\",\n"
	"\"重建 UBI 将清除出厂 MAC、U-Boot 环境与用户配置\":\"Rebuilding UBI clears the factory MAC, the U-Boot"
		" environment and user settings\",\n"
	"\"未选择 BL2 或 U-Boot，本次只写入固件\":\"No BL2 or U-Boot was picked; only the firmware goes in this time\",\n"
	"\"闪存里没有可挂载的 UBI，请先在「引导升级」里完成首次迁移\":\"There is no mountable UBI in flash — finish the first"
		" migration on Bootloader first\",\n"
	"\"闪存里没有 U-Boot（fip 卷）：写完不会自动重启，但重启之前要先到「引导升级」里补上 U-Boot 文件\":\"There is no U-Boot in flash (the"
		" fip volume). The write will not reboot on its own, but add a U-Boot file on Bootloader before"
		" you do reboot\",\n"
	"\"rootfs_data 将被清空\":\"rootfs_data will be erased\",\n"
	"\"未选择任何文件\":\"No files selected\",\n"
	"\"写入中…\":\"Writing…\",\n"
	"\"上传中…\":\"Uploading…\",\n"
	"\"传输完成，设备正在写入最后一块并擦净尾部…\":\"Transfer complete; the device is writing the last block and erasing"
		" the tail…\",\n"
	"\"传输完成，设备正在写入最后一块…\":\"Transfer complete; the device is writing the last block…\",\n"
	"\"已载入内存，即将启动\":\"Loaded into RAM, about to boot\",\n"
	"\"上传完成，设备开始写入闪存\":\"Upload complete; the device has started writing flash\",\n"
	"\"连接中断，请检查网线后重试\":\"The connection dropped. Check the cable and try again\",\n"
	"\"写入未完成，闪存已写入一部分。\":\"The write did not finish and flash is partly written. \",\n"
	"\"在重新写入成功之前不要重启设备。\":\"Do not reboot the device until a write succeeds.\",\n"
	"\"连接中断，写入可能未完成。\":\"The connection dropped and the write may be incomplete. \",\n"
	"\"未收到设备回报，请查看「诊断」中的串口日志\":\"No word back from the device. Check the serial log under Diagnostics\",\n"
	"\"通过\":\"Passed\",\n"
	"\"回读校验没通过，闪存里的内容与上传的不一致。\":\"The read-back check failed: what is in flash differs from what was"
		" uploaded. \",\n"
	"\"重新写入至成功之前不要重启设备。\":\"Do not reboot until a rewrite succeeds.\",\n"
	"\"即将完成…\":\"Almost done…\",\n"
	"\"回读校验\":\"Read-back check\",\n"
	"\"用时\":\"Took\",\n"
	"\"设备已写入完成\":\"The device finished writing\",\n"
	"\"闪存已写入。现在重启即以新内容启动；也可以留在本页面接着写别的。\":\"Flash is written. Reboot now to start from it, or stay on"
		" this page and write something else.\",\n"
	"\"闪存里仍然没有 U-Boot（fip 卷），此时重启将无法启动\":\"There is still no U-Boot in flash (the fip volume); a reboot"
		" now will not start\",\n"
	"\"写入\":\"Written\",\n"
	"\"回报\":\"Reply\",\n"
	"\"（无）\":\"(none)\",\n"
	"\"重建 UBI…\":\"Rebuilding UBI…\",\n"
	"\"写入 BL2…\":\"Writing BL2…\",\n"
	"\"写入 U-Boot…\":\"Writing U-Boot…\",\n"
	"\"写入 固件…\":\"Writing firmware…\",\n"
	"\"回读校验…\":\"Verifying…\",\n"
	"\"分区表\":\"Partition table\",\n"
	"\"可写空间\":\"Writable space\",\n"
	...
	"\"环境\":\"Env volumes\",\n"
	"\"磨损\":\"Wear\",\n"
	"\"没有找到闪存设备\":\"No flash device found\",\n"
	"\"没有分区，只能按 flash 偏移读写\":\"No partitions; only raw flash-offset access works\",\n"
	"\"bl2 分区为空。断电后 BootROM 无法找到 BL2，请在「引导升级」页上传\":\"The bl2 partition is empty. After a power-off the"
		" BootROM cannot find BL2 — upload one on Bootloader\",\n"
	"\"未设置，设备不会自行引导。可在「环境变量」页恢复默认值\":\"Not set, so the device will not boot on its own. Restore the"
		" defaults on Environment\",\n"
	"\"与当前版本默认值一致\":\"Matches this version's default\",\n"
	"\"无条目\":\"No entries\",\n"
	"\"无法挂载，闪存上无可用的 UBI。首次迁移请在「引导升级」页启用「重建 UBI」，并同时上传 BL2、U-Boot 与固件\":\"Cannot be mounted; there is no"
		" usable UBI in flash. For a first migration, turn on Rebuild UBI on the Bootloader page and"
		" upload BL2, U-Boot and the firmware together\",\n"
	"\"已挂载但无法访问\":\"Mounted but not accessible\",\n"
	"\"UBI 已挂载但无法访问\":\"UBI is mounted but not accessible\",\n"
	"\"UBI 无法挂载，只能按 flash 偏移备份\":\"UBI cannot be mounted; only raw flash-offset backups are possible\",\n"
	"\"不存在。闪存中无 U-Boot，断电后无法启动，请在「引导升级」页上传 U-Boot 文件\":\"Missing. There is no U-Boot in flash, so the"
		" device will not start after a power-off — upload a U-Boot file on Bootloader\",\n"
	"\"不存在。闪存中无固件，请在「日常刷机」页上传\":\"Missing. There is no firmware in flash — upload one on Flash"
		" firmware\",\n"
	"\"无法读取镜像描述，仅能确认为 FIT 格式\":\"The image description cannot be read; only the FIT format could be"
		" confirmed\",\n"
	"\"镜像中无描述信息\":\"The image carries no description\",\n"
	"\"无法读取，卷不存在\":\"Cannot be read; the volume does not exist\",\n"
	"\"存在，无法读取内容\":\"Present, but the contents cannot be read\",\n"
	"\"不存在。U-Boot 环境无处保存，首次正常启动时自动创建\":\"Missing. The U-Boot environment has nowhere to live; it is"
		" created at the first normal boot\",\n"
	"\"已读取\":\"Read OK\",\n"
	"\"已读取，内容为空\":\"Read OK, but empty\",\n"
	"\"已读取，无有效 MAC\":\"Read OK, but no valid MAC in it\",\n"
	"\"ethaddr 不是有效地址\":\"ethaddr is not a valid address\",\n"
	"\"未启用 CONFIG_LED，救砖时指示灯不流水\":\"CONFIG_LED is off, so the LEDs will not chase during recovery\",\n"
	...
	"\"设备正在传输备份。请等待传输完成后再上传，或刷新页面放弃该次下载\":\"The device is transferring a backup. Wait for it to finish"
		" before uploading, or reload the page to abandon that download\",\n"
	"\"设备正在接收上传，请等待写入完成后再备份\":\"The device is receiving an upload. Wait for the write to finish before"
		" backing up\",\n"
	"\"设备正忙，等这次传输完成后再扫描\":\"The device is busy; scan again once this transfer is done\",\n"
	"\"卷名不合法\":\"Invalid volume name\",\n"
	"\"卷名缺失或过长\":\"The volume name is missing or too long\",\n"
	"\"没有 Content-Length\":\"No Content-Length\",\n"
	"\"读取失败，详见串口日志\":\"Read failed — see the serial log\",\n"
	"\"起始偏移超过闪存容量\":\"The start offset is past the flash size\",\n"
	"\"偏移加长度超过闪存容量\":\"Offset plus length is past the flash size\",\n"
	...
	"\"。宽度为各卷的预留容量，按卷 ID 排列；UBI 卷在闪存中并不连续，此图不表示物理位置\":\". Widths are each volume's reserved size,"
		...
		" layout\",\n"
	"\" 个，其中 bl2 分区所在块已损坏，BootROM 可能无法读取 BL2：\":\" bad, including the block the bl2 partition sits in —"
		" the BootROM may fail to read BL2: \",\n"
	"\" MiB；写入固件时会先删掉 fit 与 rootfs_data，再腾出 \":\" MiB; writing firmware deletes fit and rootfs_data"
		" first, freeing another \",\n"
	"\"。闪存已写入一部分，此时重启将无法启动。请重新写入至成功，其间不要断电\":\". Flash is partly written and a reboot now will not"
		" start. Write again until it succeeds, and keep the power on\",\n"
	"\" 个块读取失败，该部分以 0xff 填充，详见「诊断」中的串口日志\":\" block(s) could not be read and were filled with 0xff —"
		" see the serial log under Diagnostics\",\n"
	"\"U-Boot 设备树未在 /options/u-boot 声明 \":\"The U-Boot device tree does not declare \",\n"
	"\"设备正在传输备份。请等待传输完成后再上传，或刷新页面放弃该次下载\":\"The device is transferring a backup. Wait for it to finish"
		" before uploading, or reload the page to abandon that download\",\n"
	"\"获取成功后设备位于新地址，请在上级路由的客户端列表中按 \":\"Once it has one, the device is at a new address — look it up in"
		" the upstream router's client list by \",\n"
	"\"，加上写入时会腾出的 fit 与 rootfs_data\":\", plus the fit and rootfs_data freed on write\",\n"
	"\" 不同。重建 UBI 后常见，将备份的出厂卷写回即可\":\" in the factory data. Common after a UBI rebuild — just write the"
		" backed-up factory volume back\",\n"
	"\" 处无 BL2 镜像头，可能为原厂或第三方引导程序\":\" holds no BL2 image header — it may be a stock or third-party"
		" bootloader\",\n"
	"\" 不同。两份环境内容不一致，启动时使用较新的一份\":\" — the two copies differ, and the newer one is used at boot\",\n"
	"\"环境中无 ethaddr，本次启动使用随机地址 \":\"No ethaddr in the environment; this boot uses the random address \",\n"
	"\"，超过 bl2 分区自 0x800 起可写的 \":\", more than the bl2 partition can take from 0x800 on: \",\n"
	"\"请在上级路由的客户端列表中按 MAC 查找：\":\"Look it up by MAC in the upstream router's client list: \",\n"
	"\"。当前打开本页面的主机用的就是设备发的地址。\":\". The machine viewing this page is on one of them.\",\n"
	"\"少量属正常；成片出现表明颗粒退化，应尽快备份\":\"A few are normal; a cluster of them means the die is decaying, so"
		" back up soon\",\n"
	"\"，覆盖 bootloader 及其后全部内容\":\", overwriting the bootloader and everything after it\",\n"
	"\"，接近 SLC 颗粒常见寿命，需留意坏块增长\":\", near the usual life of an SLC die — watch for new bad blocks\",\n"
	"\"本页面连接的是旧地址，不会自动恢复。请以 \":\"This page is still on the old address and will not recover by itself."
		" Reopen it at \",\n"
	"\"环境中没有这一项，当前 U-Boot 为 \":\"Not in the environment; the running U-Boot is \",\n"
	"\"。引导菜单为旧版本，下次正常启动时自动刷新\":\". The boot menu is from an older version and refreshes at the next"
		" normal boot\",\n"
	...
	"\"设备正在接收上传，请等待写入完成后再备份\":\"The device is receiving an upload. Wait for the write to finish before"
		" backing up\",\n"
	"\"偏移加镜像长度超过 flash 容量 \":\"Offset plus image length is past the flash size, \",\n"
	"\"写入 U-Boot 失败，详见串口日志\":\"Writing U-Boot failed — see the serial log\",\n"
	"\"没有分区，只能按 flash 偏移读写\":\"No partitions; only raw flash-offset access works\",\n"
	"\"不保存：只管本次开机，下次开机回到 \":\"Not saved: this lasts until the next power-up, which goes back to \",\n"
	"\"设备启动时的菜单，序号即串口上按的键\":\"The menu shown at boot; the number is the key pressed on the serial"
		" console\",\n"
	"\"本页面将断开，需以 http://\":\"This page will drop; reopen it at http://\",\n"
	"\"偏移加长度超过 flash 容量 \":\"Offset plus length is past the flash size, \",\n"
	"\" 秒内无按键则执行 bootcmd\":\" s without a keypress runs bootcmd\",\n"
	"\"），请重新上传 U-Boot 文件\":\"). Upload the U-Boot file again\",\n"
	"\"），卷内数据校验未通过，内容已损坏\":\"), and the volume's checksum failed — the contents are corrupt\",\n"
	"\"没有 Content-Length\":\"No Content-Length\",\n"
	"\"：本机这个地址马上就没人续租了，\":\": nothing will renew your computer's address any more, so \",\n"
	"\" 查找。未取得租约时退回原地址。\":\". Without a lease it falls back to the old address.\",\n"
	"\"起始偏移超出 flash 容量 \":\"The start offset is past the flash size, \",\n"
	"\"当前主机用的是手动配置的 IP。\":\"your computer is on a manually configured IP.\",\n"
	"\"与当前版本默认值不同，已被修改：\":\"Differs from this version's default; modified: \",\n"
	"\"。下次正常启动时自动刷新引导菜单\":\". The boot menu refreshes at the next normal boot\",\n"
	"\"写入 BL2 失败，详见串口日志\":\"Writing BL2 failed — see the serial log\",\n"
	...
	"\" · 已截断，完整内容见串口\":\" · truncated; the full list is on the serial console\",\n"
	"\"卷内不是 FIT 镜像（头 \":\"The volume does not hold a FIT image (header \",\n"
	"\"，与当前 U-Boot 一致\":\", matches the running U-Boot\",\n"
	"\" 读回来的内容与上传的不一致\":\" read back differently from what was uploaded\",\n"
	"\"自 flash 偏移 0x\":\"Writes from flash offset 0x\",\n"
	"\" 块，镜像中对应内容未写入\":\" block(s); the matching parts of the image were not written\",\n"
	"\" 个，已由 UBI 避开：\":\" bad, all avoided by UBI: \",\n"
	"\"，当前 U-Boot 为 \":\", the running U-Boot is \",\n"
	"\"写入固件失败，详见串口日志\":\"Writing the firmware failed — see the serial log\",\n"
	"\"整片写入失败，详见串口日志\":\"The whole-chip write failed — see the serial log\",\n"
	"\"内存不足，无法分配读取窗口\":\"Not enough memory for a read window\",\n"
	"\" 不存在时按文件长度创建\":\" will be created at the file's size if it does not exist\",\n"
	"\"，超过设备单次可接收的 \":\", more than the device can take in one go: \",\n"
	"\" 字节，超过它们覆盖的 \":\" bytes, more than the \",\n"
	"\"（本次读取已纠正位翻转）\":\" (bit flips were corrected on this read)\",\n"
	"\" 次地址，均未被接受；\":\" offer(s), none accepted; \",\n"
	"\"空闲与 UBI 预留 \":\"Free and UBI reserve \",\n"
	"\"，超过 fip 卷的 \":\", more than the fip volume's \",\n"
	"\"：内存不足，设备将拒绝\":\" — not enough RAM, so it will be refused\",\n"
	"\" 字节没有划进任何分区\":\" bytes not covered by any partition\",\n"
	"\"bl2 分区读取失败（\":\"Reading the bl2 partition failed (\",\n"
	"\" 项，当前版本默认为 \":\" entries; this version defaults to \",\n"
	"\"卷内不是 FIP（头 \":\"The volume does not hold a FIP (header \",\n"
	"\" 个灯，一个也没有找到\":\" LEDs and not one was found\",\n"
	"\" 卷失败，详见串口日志\":\" volume failed — see the serial log\",\n"
	"\" 字节将超出闪存容量 \":\" bytes would run past the flash size of \",\n"
	"\"读取失败，详见串口日志\":\"Read failed - see the serial log\",\n"
	"\"偏移加长度超过闪存容量\":\"Offset plus length is past the flash size\",\n"
	"\"设备拒绝了本次备份：\":\"The device refused this backup: \",\n"
	"\"。红色条目会写入闪存\":\". Red entries write to flash\",\n"
	"\" MiB：当前空闲 \":\" MiB — free now \",\n"
	"\" 页在读取时被纠正。\":\" page(s) were corrected on read. \",\n"
	"\"写入偏移未按擦除块 \":\"The write offset is not aligned to the erase block, \",\n"
	"\"U-Boot 文件 \":\"The U-Boot file is \",\n"
	"\" 处有 BL2 镜像\":\" holds a BL2 image\",\n"
	"\"，救砖时指示灯不流水\":\" under /options/u-boot, so the LEDs will not chase during recovery\",\n"
	"\"起始偏移超过闪存容量\":\"The start offset is past the flash size\",\n"
	"\"需手动把本机配成 \":\"set it manually to an address in \",\n"
	"\"，最近一次分配给 \":\", most recently to \",\n"
	"\" 保留原有内容不动\":\" after the image keeps what is already there\",\n"
	"\" 的分区，无法检查\":\" among the partitions, so it cannot be checked\",\n"
	"\"），请重新上传固件\":\"). Upload the firmware again\",\n"
	"\"，与出厂数据中的 \":\", differs from \",\n"
	"\"DHCP 服务器\":\"DHCP server\",\n"
	"\"DHCP 客户端\":\"DHCP client\",\n"
	"\"正在读取并传送 \":\"Reading and sending \",\n"
	"\" · 写入时腾出\":\" · freed on write\",\n"
	"\"镜像之后剩余的 \":\"The \",\n"
	"\"回读校验没通过：\":\"The read-back check failed: \",\n"
	"\"设备拒绝了上传（\":\"The device refused the upload (\",\n"
	"\"声明的大小合计 \":\"declared sizes total \",\n"
	"\" MiB 未划分\":\" MiB is unpartitioned\",\n"
	"\" 个逻辑擦除块）\":\" logical erase blocks)\",\n"
	"\" 字节，无法读取\":\" bytes and cannot be read\",\n"
	"\" 字节，卷内仅 \":\" bytes but the volume holds only \",\n"
	"\" 字节，校验通过\":\" bytes, checksum OK\",\n"
	"\"，与出厂数据一致\":\", matches the factory data\",\n"
	"\"已读取，MAC \":\"Read OK, MAC \",\n"
	"\"（闪存尚未改动）\":\" (flash was not modified)\",\n"
	"\"没有找到闪存设备\":\"No flash device found\",\n"
	"\" 个文件 · \":\" file(s) · \",\n"
	"\"设备没有接受：\":\"The device did not accept it: \",\n"
	"\"下次开机回到 \":\"The next power-up goes back to \",\n"
	"\"UBI 预留 \":\"UBI reserve \",\n"
	"\" · 擦除块 \":\" · erase block \",\n"
	"\" 项异常 · \":\" fail · \",\n"
	"\" 项注意 · \":\" warn · \",\n"
	"\"扫描没能进行（\":\"The scan could not run (\",\n"
	"\" 将被擦成空白\":\" after the image will be erased blank\",\n"
	"\"BL2 文件 \":\"The BL2 file is \",\n"
	"\"设备写入失败（\":\"The device failed to write (\",\n"
	"\" 个卷，坏块 \":\" volumes, \",\n"
	"\" 个逻辑擦除块\":\" free logical erase blocks\",\n"
	"\"擦写次数最大 \":\"Erase count max \",\n"
	"\"FIT 镜像，\":\"FIT image, \",\n"
	"\"存在，CRC \":\"Present, CRC \",\n"
	"\" 个没有找到）\":\" not found)\",\n"
	"\" 未按擦除块 \":\" is not aligned to the erase block \",\n"
	"\"卷名缺失或过长\":\"The volume name is missing or too long\",\n"
	"\"（出厂默认）\":\" (factory default)\",\n"
	"\" · 电脑 \":\" · computer \",\n"
	"\"/ 重新打开\":\"/\",\n"
	"\" 的一个地址\":\"\",\n"
	"\"设备已移至 \":\"The device moved to \",\n"
	"\" 重新打开。\":\".\",\n"
	"\" 块读取失败\":\" unreadable block(s)\",\n"
	"\"空闲与预留 \":\"Free and reserve \",\n"
	"\"逻辑擦除块 \":\"Logical erase block \",\n"
	"\"。刷机可用 \":\". Available for flashing: \",\n"
	"\" · 剩余 \":\" · left \",\n"
	"\" 页无法读出\":\" page(s) could not be read\",\n"
	"\"连续覆盖整片\":\"contiguous over the whole chip\",\n"
	"\" 个，空闲 \":\" bad blocks, \",\n"
	"\"。当前空闲 \":\". Free now \",\n"
	"\"，实际写入 \":\", wrote \",\n"
	"\"请求头超过 \":\"The request header is longer than \",\n"
	"\"下次开机：\":\"Next power-up: \",\n"
	"\"设置失败（\":\"Arming failed (\",\n"
	"\"读至片尾，\":\"Reads to the end of the chip, \",\n"
	"\"传输完成；\":\"Transfer complete; \",\n"
	"\" 起的区段\":\" onwards\",\n"
	"\"恢复失败（\":\"Restore failed (\",\n"
	"\" · 页 \":\" · page \",\n"
	...
	"\"预计还需 \":\"About \",\n"
	"\"回读校验 \":\"Verifying \",\n"
	"\"没有名为 \":\"There is nothing named \",\n"
	"\" 个分区，\":\" partitions; \",\n"
	"\" 之间有 \":\" has \",\n"
	"\"连续覆盖 \":\"contiguous over \",\n"
	"\"片上其余 \":\"the chip's other \",\n"
	"\"，擦除块 \":\", erase block \",\n"
	"\"，OOB \":\", OOB \",\n"
	"\"刷机可用 \":\"Available for flashing: \",\n"
	"\"闪存中为 \":\"Flash holds \",\n"
	"\"镜像声明 \":\"The image declares \",\n"
	"\"存在，与 \":\"Present, matches \",\n"
	"\" 声明了 \":\" declares \",\n"
	"\"（声明了 \":\" (declares \",\n"
	"\" 个，有 \":\", \",\n"
	"\"写入偏移 \":\"The write offset \",\n"
	"\"卷名不合法\":\"Invalid volume name\",\n"
	"\"长度为 0\":\"The length is 0\",\n"
	"\"静态地址\":\"Static address\",\n"
	"\"已发出 \":\"Handed out \",\n"
	...
	"\" 项正常\":\" ok\",\n"
	"\" 字节）\":\" bytes)\",\n"
	"\"：有重叠\":\" they span: they overlap\",\n"
	"\"擦除块 \":\"erase block \",\n"
	"\" 失败（\":\" failed (\",\n"
	"\"读不回 \":\"could not read back \",\n"
	"\"已选 \":\"Selected \",\n"
	"\"设备 \":\"Device \",\n"
	"\"端口 \":\"Port \",\n"
	"\"空闲 \":\"Free \",\n"
	...
	"};\n"
	"/* i18n-dict end */\n"
	"var LANG='zh',I18RE=null,I18R=null,I18ON=0,\n"
	"I18A=['title','aria-label','placeholder','data-l'],\n"
	"CJK=/[\\u4e00-\\u9fff]/;\n"
	"function i18re(){var k=[],x;for(x in I18P)k.push(x);\n"
	"k.sort(function(a,b){return b.length-a.length});\n"
	"I18RE=k.length?new RegExp(k.map(function(s){\n"
	"return s.replace(/[.*+?^${}()|[\\]\\\\]/g,'\\\\$&')}).join('|'),'g'):null}\n"
	"/* 诊断导出的那几行不进 DOM，只能显式过一道 */\n"
	"function T(s){return LANG=='en'?i18s(s):s}\n"
	"function i18s(s){var e=I18N[s];if(e!==undefined)return e;\n"
	"return I18RE?s.replace(I18RE,function(m){return I18P[m]}):s}\n"
	"function i18text(n){var s=n.data,r;if(!s||!CJK.test(s))return;\n"
	"r=i18s(s);if(r!=s){if(n.zh0===undefined)n.zh0=s;n.data=r}}\n"
	"function i18attr(e,a){var s=e.getAttribute&&e.getAttribute(a),r;\n"
	"if(!s||!CJK.test(s))return;\n"
	"r=i18s(s);if(r!=s){if(e['zh0_'+a]===undefined)e['zh0_'+a]=s;e.setAttribute(a,r)}}\n"
	"/* script 与 style 的正文也是文本节点，注释里全是中文，别去动它 */\n"
	"function i18skip(n){var t=n.tagName;\n"
	"return t=='SCRIPT'||t=='STYLE'||n.hasAttribute('data-raw')}\n"
	"function i18on(n){var c;\n"
	"if(n.nodeType==3){i18text(n);return}\n"
	"if(n.nodeType!=1||i18skip(n))return;\n"
	"I18A.forEach(function(a){if(n.hasAttribute(a))i18attr(n,a)});\n"
	"for(c=n.firstChild;c;c=c.nextSibling)i18on(c)}\n"
	"/*\n"
	"* 还原以节点上存的原文为准。但有几处是把当时的文字先存进变量、过后再写回去的\n"
	"* （复制按钮、提交按钮的原标题），那份文字在英文下存的就是英文，写回来的是个\n"
	"* 新节点、身上没有原文。对这种整段相等的再按英文反查一次 —— 只认整段，不碰\n"
	"* 片段，设备日志里的英文单词才不会被换掉。\n"
	"*/\n"
	"function i18rev(){I18R={};for(var k in I18N)I18R[I18N[k]]=k}\n"
	"function i18off(n){var c,b;\n"
	"if(n.nodeType==3){if(n.zh0!==undefined){n.data=n.zh0;n.zh0=undefined}\n"
	"else if(I18R&&(b=I18R[n.data])!==undefined)n.data=b;\n"
	"return}\n"
	"if(n.nodeType!=1||i18skip(n))return;\n"
	...
	"try{localStorage.setItem('xglang',l)}catch(e){}\n"
	"document.documentElement.setAttribute('lang',l);\n"
	"if(l=='en'){if(!I18RE)i18re();I18ON=1;i18on(document.body);\n"
	"if(I18OB)I18OB.observe(document.body,{childList:true,subtree:true,\n"
	"characterData:true,attributes:true,attributeFilter:I18A})}\n"
	"else{I18ON=0;if(I18OB)I18OB.disconnect();if(!I18R)i18rev();i18off(document.body)}}\n"
	"/* 没选过就照浏览器的语言挑一次；点过之后以点的为准 */\n"
	"try{if((localStorage.getItem('xglang')||(/^zh/i.test(navigator.language||'')?'zh':'en'))=='en')s"
		"etlang('en')}catch(e){}\n"
	"bind();info();hbstart();\n"
	"</script>\n"
	"</body></html>\n"
