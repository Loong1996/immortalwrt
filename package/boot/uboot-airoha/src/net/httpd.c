...
/* @@PAGE_BEGIN@@ */
	...
	"var INFO=null,FV=[],SENT=null,CHK=null,ENV=null,YES=null,INFOQ=0,CHKQ=0,ENVQ=0,HB=0,HBT=null,HBG"
		...
		" \"+sz(INFO.flash.page):\"\"],[\"分区\",(INFO.parts||[]).map(function(e){return e.n+\""
		...
		"urn\"bl2\"==e.k}),m=t.some(function(e){return\"chain\"==e.k}),v=t.some(function(e){return\"firmware\"="
		"=e.k}),g=chain()&&INFO.chain.bootcmd;if(chain()){if(u&&!v&&n.push(\"打开了「重建"
		" UBI」却没有选择固件：重建之后闪存里没有系统可以启动\"),INFO&&!INFO.ubi&&v&&!u&&n.push(\"闪存里没有可挂载的 UBI：请打开「重建"
		" UBI」，并同时上传固件\"),u&&o.push(\"重建 UBI 将清除 U-Boot 环境与用户配置；factory 卷随即从原厂 DSD"
		" 区重新生成\"),m){o.push(\"U-Boot 写入 chainloader 分区，写完回读校验；写坏了原厂引导起不来它，只能接串口救\"),g&&!/0x60(0000|2100)(?!"
		"[0-9a-fA-F])/.test(g)?o.push(\"原厂 bootcmd 不读 0x600000 或 0x602100，原厂引导不会启动写进去的"
		" U-Boot：\"+esc(g)):g&&!/0x602100(?![0-9a-fA-F])/.test(g)&&o.push(\"原厂 bootcmd 从 0x600000 读，要经前缀"
		" shim 启动，这条路没有实机验证过：\"+esc(g));var w=INFO.chain.rd,B=t.filter(function(e){return\"chain\"==e.k})[0]"
		".f,I=0;w&&((I=6291456==w[0]?B.size:6299904==w[0]?B.size-8448:0)&&w[1]<I&&n.push(\"原厂 bootcmd 只读"
		" \"+sz(w[1])+\"，这份 U-Boot 要 \"+sz(I)+\"：读不全，原厂引导起不来它\"),6291456==w[0]&&2172649472!=w[2]&&n.push(\"原厂"
		" bootcmd 读到 0x\"+w[2].toString(16)+\"，前缀 shim 只认 0x81800000\"))}}else u&&!p&&n.push(\"打开了「重建"
		" UBI」却没有选择 U-Boot 文件：重建会抹掉 fip 卷，没有 U-Boot 设备将无法启动\"),u&&!b&&n.push(\"打开了「重建 UBI」却没有选择 BL2：重建从"
		" 0x20000 起擦，盖住了原厂引导器的后半截，只写 U-Boot 的话重启起不来，只能拆串口救\"),INFO&&INFO.ubi&&!INFO.ubi.fip&&!p&&n.push(\"闪"
		"存里没有 U-Boot（fip 卷），本次必须同时上传 U-Boot 文件\"),!INFO||INFO.ubi||u||n.push(\"闪存里没有可挂载的 UBI：请打开「重建"
		" UBI」，并同时上传 BL2、U-Boot 与固件\"),u&&o.push(\"重建 UBI 将清除出厂 MAC、U-Boot"
		" 环境与用户配置\");if(u)if(INFO&&INFO.ubi){var k=bkmissing();k.length&&o.unshift(esc(k.join(\"、\"))+' 未备份"
		" · <a href=\"#\" onclick=\"gobk();return false\">去备份</a>')}else bkall().all||o.unshift('原厂系统请先整片备份"
		" · <a href=\"#\" onclick=\"gobk();return false\">整片下载</a>');(chain()?m:b||p)||o.push(chain()?\"未选择"
		" U-Boot，本次只写入固件\":\"未选择 BL2 或 U-Boot，本次只写入固件\")}var"
		" y=null,T=null,x=$(\"[name=format]\",e)&&$(\"[name=format]\",e).checked;(INFO&&INFO.parts||[]).forEa"
		"ch(function(e){\"bl2\"==e.n&&(y=e)}),(INFO&&INFO.ubi&&INFO.ubi.vols||[]).forEach(function(e){\"fip\""
		"==e.n&&(T=e)});var N=T&&!x?T.s:1048576;t.forEach(function(e){\"bl2\"==e.k&&y&&e.f.size>y.s-2048&&n"
		".push(\"BL2 文件 \"+sz(e.f.size)+\"，超过 bl2 分区自 0x800 起可写的"
		" \"+sz(y.s-2048)),\"fip\"==e.k&&N&&e.f.size>N&&n.push(\"U-Boot 文件 \"+sz(e.f.size)+\"，超过 fip 卷的"
		" \"+sz(N)),\"chain\"==e.k&&chain()&&e.f.size>INFO.chain.s&&n.push(\"U-Boot 文件 \"+sz(e.f.size)+\"，超过"
		...
		" 将被清空\");var U=0;t.forEach(function(e){U+=e.f.size}),\"p4\"!=e.id&&INFO&&INFO.uploadmax&&U+4096>INF"
		"O.uploadmax&&n.push(\"本次上传 \"+sz(U)+\"，超过设备单次可接收的 \"+sz(INFO.uploadmax)+\"：内存不足，设备将拒绝\");var"
