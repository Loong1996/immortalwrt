 ...
 * and data segments held back a little so they arrive out of order.
 *
 * The peer has a receive buffer of WIN bytes that its application empties
 * at READ bytes per millisecond (0: at once), advertises what is free, and
 * drops what lands past that, like a real stack.  Its SYN offers a window scale of WSCALE (-1: none).
 * The device's SYN-ACK offers none, so by RFC 7323 the window the peer
 * advertises is never scaled -- a device that shifts it anyway sends past
 * the window, and "overwin" counts the segments that did.
 *
 * usage: dl TOTAL LOSS ACKLOSS SEED DELACK_MS [WIN [WSCALE [REORDER [READ]]]]
 */
#include <net.h>
#include <net/tcp.h>
#include <stdlib.h>
...
static int DELACK_MS, WSCALE = -1;
static u32 WIN = 65535;
static double REORDER, READ;
static unsigned long data_bytes_sent, segs_sent, data_errors, overwin;
			...
			int sport, u8 action, u32 seq, u32 ack)
{
	...
	if (payload_len) {
		...
		if (rnd(LOSS))
			return 0;
		/* held up on the way: whatever is sent next overtakes it */
		if (rnd(REORDER))
			p.at += 200 + (unsigned long long)(drand48() * 800);
	}
	enqueue(&p);
	return 0;
}
...
static void deliver_to_dev(struct pkt *p)
{
	...
	if ((p->flags & TCP_SYN) && WSCALE >= 0) {
		uchar *o = (uchar *)&b + IP_TCP_HDR_SIZE;

		o[0] = TCP_1_NOP;
		o[1] = TCP_O_SCL;
		o[2] = 3;
		o[3] = WSCALE;
		b.ip.hdr.tcp_hlen = 0x60;
		pkt_len += 4;
	}
...
static double p_read;	/* how far the application has read */

/* Free buffer space: the window to advertise, and to accept data into. */
static u32 p_free(void)
{
	u32 cap = WIN < 65535 ? WIN : 65535;

	if (!READ || p_read > p_rcv)
		p_read = p_rcv;
	return cap - (u32)(p_rcv - p_read);
}
	...
	if (p->len) {
		...
		for (i = 0; i < p->len; i++)
			if (p->data[i] != pat(off + i))
				data_errors++;
		/*
		 * RFC 9293 3.10.7.4: a segment is acceptable when it starts
		 * inside the window, and what runs past it is cut off; one
		 * that starts past it is dropped.  Only that counts as sent
		 * past the window -- a sender may always probe with one
		 * segment into a window smaller than a segment.
		 */
		edge = p_rcv + p_free();
		if (off >= edge && off + p->len > p_rcv) {
			overwin++;
			peer_send(TCP_ACK);
			return;
		}
		len = off + p->len > edge ? edge - off : p->len;
		if (off + len <= p_rcv) {		/* duplicate */
			peer_send(TCP_ACK);
		} else if (off > p_rcv) {		/* out of order */
			memset(got + off, 1, len);
			peer_send(TCP_ACK);
		} else {
			memset(got + off, 1, len);
			before = p_rcv;
...
int main(int argc, char **argv)
{
	...
	WIN = argc > 6 ? strtoul(argv[6], NULL, 0) : 65535;
	WSCALE = argc > 7 ? atoi(argv[7]) : -1;
	REORDER = argc > 8 ? atof(argv[8]) : 0;
	READ = argc > 9 ? atof(argv[9]) : 0;
	...
	while (now_us < limit && p_done == 0) {
		...
		if (READ) {
			...
			p_read += READ * 5 / 1000;
			/* a window that opens from nothing is announced */
			if (before < 1448 && p_free() >= 1448 && !p_fin_in)
				peer_send(TCP_ACK);
		}
	}
	...
	printf("%s: %u bytes in %.3f s = %.1f KiB/s, segs %lu, resent %.2f%%, "
	       "errors %lu, peer %s, dev %s (status %d), overwin %lu\n",
	       ...
	       dev_closed ? "closed" : "OPEN", dev_status, overwin);
