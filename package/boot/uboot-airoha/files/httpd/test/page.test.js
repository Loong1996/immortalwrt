...
(async () => {
  ...
  console.log('\n--- 直接启动系统 / 下次开机进恢复页 ---');
  {
    ...
    w.askboot();
    ok('要确认', on(w, '#mask') && /run boot_production/.test(txt(w, '#abody')));
  ...
  console.log('\n--- 并口 NAND：带 OOB 的整片备份与原厂格式 ---');
  {
    ...
    const folds = [...w.document.querySelectorAll('.fold')];
    ok('识别出来时两处参数框都收起', folds.length === 2 && folds.every(d => !d.open), folds.map(d => d.open).join());
    ok('收起那一行写着这组参数', [...w.document.querySelectorAll('.fsv')].every(e =>
       e.textContent === 'ECC8 · spare 28 · 数据取反关 · FDM 8 字节，其中 1 字节参与校验 · 坏块标记交换关'), txt(w, '.fsv'));
    ok('参数框填的是识别结果',
    ...
    ok('参数不成立时参数框自己展开', folds.every(d => d.open));
    ok('收起那一行跟着改', /^ECC16 /.test(txt(w, '.fsv')), txt(w, '.fsv'));
    ...
    ok('dd 核对是自动识别旁边的一个按钮', $(w, '#sfa').nextElementSibling === $(w, '#sfb') && /用 dd 备份核对/.test(txt(w, '#sfb')));
    $(w, '#sfdd').onchange();
    ok('选完 dd 文件就开始核对', /正在/.test(txt(w, '#sfh')) && $(w, '#sfb').disabled, txt(w, '#sfh'));
    ok('核对期间其余按钮也灰掉，免得结果互相盖掉', ['#sfa', '#sft', '#sfs'].every(id => $(w, id).disabled));
    await sleep(3000);
    ok('dd 核对一致', /一致/.test(txt(w, '#sfh')) && /ok/.test($(w, '#sfh').className), txt(w, '#sfh'));
    ok('核对完按钮恢复', ['#sfa', '#sfb', '#sft'].every(id => !$(w, id).disabled));
    ...
    ok('被挪过的块列出来、标橙', /2 个块对不上：0x2a00000、0x9c00000/.test(txt(w, '#sfh')) &&
       /warn/.test($(w, '#sfh').className), txt(w, '#sfh'));
  ...
  }
  {
    ...
    const fs2 = [...w.document.querySelectorAll('.fold')];
    ok('识别不出唯一一组时参数框展开', fs2.every(d => d.open));
    fs2.forEach(d => { d.open = false; });
    const e2 = $(w, '#sfp1 select[data-k=ecc]'); e2.value = '16'; e2.onchange();
    ok('参数又不成立时再展开一次', fs2.every(d => d.open));
    fs2.forEach(d => { d.open = false; });
    w.info(); await sleep(800);
    ok('两种情形都在时收起后不再来回弹开', fs2.every(d => !d.open));
    $(w, '#sfp1 select[data-k=ecc]').value = '8'; $(w, '#sfp1 select[data-k=ecc]').onchange();
    $(w, '.nav[data-p=p2]').click();
    $(w, '#p2 [name=format]').checked = true;
    w.ask();
  ...
  console.log('\n--- UBI 被拒：补丁 205 不肯挂，页面给出路 ---');
  {
    const w = await boot();
    const b = () => txt(w, '#bant');
    const zh = t => /[\u4e00-\u9fff]/.test(t);

    /* 混合状态：扫到 5% 停下，卷表读得出，强制挂载能挂上 */
    setsel(w, 'dev', 'ubiecc');
    await sleep(800);
    ok('横幅说有一部分是别的 ECC 格式', /有一部分数据不是按本 U-Boot 的 ECC 格式写的/.test(b()), b().slice(0, 60));
    ok('横幅给出块数、说明什么都没擦', /至少 102 块，为保护数据一块都没擦/.test(b()), b());
    ok('横幅里有强制挂载', !!$(w, '#bant a[onclick^=askforce]'));
    w.askforce();
    ok('确认框写明要擦的块', /至少 102 块/.test(txt(w, '#abody')) && /找不回/.test(txt(w, '#abody')),
       txt(w, '#abody').slice(0, 80));
    ok('确认框提醒还没整片备份', /还没有整片备份/.test(txt(w, '#abody')));
    ok('确认框说挂上后要先重启', /挂上后先重启一次/.test(txt(w, '#abody')));
    ok('确认按钮是强制挂载', txt(w, '#yes') === '强制挂载', txt(w, '#yes'));
    w.go();
    ok('等待时横幅说设备不响应', /设备不响应/.test(b()), b());
    await sleep(1200);
    ok('挂上以后横幅消失', $(w, '#ban').hasAttribute('hidden'), b());
    ok('挂上以后弹框要求先重启', on(w, '#mask') && txt(w, '#atitle') === '已挂载 UBI' && txt(w, '#yes') === '重启',
       txt(w, '#atitle') + ' / ' + txt(w, '#yes'));
    ok('弹框说明默认环境会盖掉闪存里那份', /盖掉闪存里原来那份/.test(txt(w, '#abody')));
    w.hide();

    /* 整片别的格式：抽样就拒，卷表也读不出，不给强制挂载 */
    setsel(w, 'dev', 'ubieccall');
    await sleep(800);
    ok('整片别的格式另有说法', /闪存上的数据不是按本 U-Boot 的 ECC 格式写的/.test(b()) && /卷表也读不出来/.test(b()), b());
    ok('整片别的格式不给强制挂载', !$(w, '#bant a[onclick^=askforce]'));
    w.askforce();
    ok('直接调也不弹框', !on(w, '#mask'));

    /* 体检那一栏跟横幅说一样的话，不再叫人去重建 */
    setsel(w, 'dev', 'ubiecc');
    await sleep(800);
    w.check();
    await sleep(2000);
    ok('体检说没有挂载、没擦', /没有挂载：至少 102 块是按别的 ECC 格式写的/.test(txt(w, '#chk')) &&
       !/首次迁移请/.test(txt(w, '#chk')), txt(w, '#chk').slice(0, 120));
    ...
    w.go();
    await sleep(1500);
    ok('断线时说结果未知', /连接中断，挂载结果未知/.test(b()), b());
    setsel(w, 'conn', 'up');
    await sleep(3500);

    /* 原厂布局：挂得上，但原厂 UBI 里没有 fip */
    setsel(w, 'dev', 'ubiforeign');
    await sleep(800);
    ok('原厂数据那种另有说法', /没有 fip 卷/.test(b()) && /一块都没擦/.test(b()), b());
    ok('断线那句换了拒绝就不再挂着', !/连接中断/.test(b()), b());
    w.askforce();
    ok('确认框提到原厂分区', /原厂分区/.test(txt(w, '#abody')));
    w.go();
    await sleep(1200);
    w.hide();
    await sleep(200);
    ok('挂上后横幅改说没有 fip 卷', /没有 U-Boot（fip 卷）/.test(b()) && !/UBI 没有挂载/.test(b()), b());
    ...
    w.askforce();
    await sleep(200);
    ok('英文确认框没有中文', !zh(txt(w, '#abody')) && !zh(txt(w, '#atitle')),
       txt(w, '#atitle') + ' | ' + txt(w, '#abody').slice(0, 80));
    w.go();
    await sleep(1400);
    ok('英文重启提示没有中文', !zh(txt(w, '#abody')) && !zh(txt(w, '#atitle')),
       txt(w, '#atitle') + ' | ' + txt(w, '#abody').slice(0, 80));
    w.hide();
    setsel(w, 'dev', 'ubieccall');
    await sleep(800);
    ok('英文整片别的格式没有中文', !zh(b()), b());
    w.check();
    await sleep(2000);
    ok('英文体检没有中文', !zh(txt(w, '#chk')), txt(w, '#chk').slice(0, 120));
    await sleep(200);
    ok('英文界面没有中文残留', zhLeft(w).length === 0, zhLeft(w).join(' | '));
    ok('没有脚本错误', w.__errs.length === 0, w.__errs.join(' | '));
  }
