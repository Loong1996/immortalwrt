...
document.addEventListener('DOMContentLoaded',function(){
 ...
 b.innerHTML='<b>预览</b> 设备 <select id=pvdev><option value=ok>正常</option><option value=noubi>没有 UBI</option><option value=nofip>没有 fip 卷</option><option value=nolog>不带串口日志</option><option value=noinfo>/info 失败</option><option value=info500>/info 回 500</option><option value=chain>原厂引导锁着（XR1710G）</option><option value=chainnoubi>原厂引导锁着，没有 UBI</option></select> 提交 <select id=pvpost><option value=ok>成功</option><option value=reject>设备拒绝 400</option><option value=fail500>写到一半失败 500</option><option value=drop>断线</option><option value=busy>另一个上传占着</option></select> 连接 <select id=pvconn><option value=up>正常</option><option value=down>断开</option></select>';
